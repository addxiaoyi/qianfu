...
export function createWAFMiddleware(config: WAFConfig) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const deny = (statusCode: number, message: string, code: string) =>
    ...
    if (isBanned) {
      ...
      try {
        ...
        await logAction(uid, 'WAF_BLOCK', 'ip_ban', req as any, { request_id: rid, ip: clientIP, path: req.path });
      } catch {}
    ...
    try {
      ...
      const vNum = vStr ? parseInt(vStr as any, 10) : 0;
      if (vNum >= 2) {
        (req as any).graylisted = true;
    ...
    for (const pattern of badUserAgents) {
      if (pattern.test(userAgent)) {
        ...
        try {
          const uid = (req as any).user?.id ?? null;
          const rid = (req as any).requestId ?? '';
          await logAction(uid, 'WAF_BLOCK', 'bad_user_agent', req as any, { request_id: rid, ip: clientIP, userAgent, path: req.path });
        } catch {}
        ...
        for (const pattern of suspiciousPatterns) {
          if (pattern.test(value)) {
            ...
            try {
              const uid = (req as any).user?.id ?? null;
              const rid = (req as any).requestId ?? '';
              await logAction(uid, 'WAF_BLOCK', 'bad_header', req as any, { request_id: rid, ip: clientIP, header: headerName, path: req.path });
            } catch {}
    ...
    if (config.blockSuspiciousIPs) {
      ...
      if (count > config.maxRequestsPerWindow) {
        ...
        try {
          const uid = (req as any).user?.id ?? null;
          const rid = (req as any).requestId ?? '';
          await logAction(uid, 'WAF_RATE_LIMIT', 'global', req as any, { request_id: rid, ip: clientIP });
        } catch {}
      ...
      if (isSensitive) {
        ...
        if (sensitiveCount > maxSensitiveRequests) {
          ...
          try {
            const uid = (req as any).user?.id ?? null;
            const rid = (req as any).requestId ?? '';
            await logAction(uid, 'WAF_RATE_LIMIT', 'sensitive', req as any, { request_id: rid, ip: clientIP, path: req.path });
          } catch {}
    ...
    for (const pattern of suspiciousPatterns) {
      if (pattern.test(req.path) || pattern.test(req.url)) {
        ...
        try {
          const uid = (req as any).user?.id ?? null;
          const rid = (req as any).requestId ?? '';
          await logAction(uid, 'WAF_BLOCK', 'path_or_url', req as any, { request_id: rid, ip: clientIP, path: req.path, url: req.url });
        } catch {}
        ...
        for (const pattern of suspiciousPatterns) {
          if (pattern.test(checkStr)) {
            ...
            try {
              const uid = (req as any).user?.id ?? null;
              const rid = (req as any).requestId ?? '';
              await logAction(uid, 'WAF_BLOCK', 'body_content', req as any, { request_id: rid, ip: clientIP, path: req.path });
            } catch {}
