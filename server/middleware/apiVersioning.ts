 ...
 */
export const createVersionedRouter = (version: ApiVersion | string) => {
  const config = versionConfigs[version];
  if (!config) {
 ...
 * 获取所有已注册的版本配置
 */
export const getVersionConfigs = (): Readonly<typeof versionConfigs> => {
 ...
 * @param config 版本配置
 */
export const registerVersion = (version: string, config: VersionConfig): void => {
 ...
 */
export const backwardCompatRedirect = (
  req: Request,
  res: Response,
  next: NextFunction,
): void => {
