...
export interface AuditReport {
  ...
  top_users: Array<{ user_id: number; event_count: number; username?: string }>;
  anomalies: Array<{
    type: string;
    description: string;
    severity: string;
...
export class AuditAnalyzer {
  ...
  // Detect rapid role changes
  private static detectRapidRoleChanges(events: AuditEvent[]): {
    type: string;
    description: string;
    severity: string;
    count: number;
  } {
    const roleChangeEvents = events.filter(event => 
      AUDIT_CONFIG.EVENTS.PERMISSION_CHANGES.includes(event.action)
    );

    // Group by user, detect multiple changes in short time
    const userChanges = new Map<number, number>();
  ...
  // Detect unusual access times
  private static detectUnusualAccessTimes(events: AuditEvent[]): {
    type: string;
    description: string;
    severity: string;
    count: number;
  ...
  // Detect bulk permission modifications
  private static detectBulkPermissionModifications(events: AuditEvent[]): {
    type: string;
    description: string;
    severity: string;
    count: number;
  } {
    const bulkOperations = events.filter(event => 
      event.action === 'BATCH_PERMISSION_GROUP_ASSIGNED' ||
      event.action === 'BATCH_REVIEW_COMPLETED'
    ).length;
  ...
  private static generateRecommendations(
    anomalies: Array<{ type: string; severity: string; count: number }>,
