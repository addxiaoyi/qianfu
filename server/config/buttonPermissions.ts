...
export interface ButtonPermission {
  ...
  description: string;
  category: string;
  dangerLevel: 0 | 1 | 2 | 3; // 0=安全, 1=低, 2=中, 3=高危险
}
...
export class ButtonPermissionManager {
  private static permissionMap = new Map(Object.entries(BUTTON_PERMISSIONS));
  ...
  static getPermissionsByCategory(): Record<string, ButtonPermission[]> {
    ...
    for (const [id, perm] of Object.entries(BUTTON_PERMISSIONS)) {
      ...
      grouped[perm.category].push({
        id: id as ButtonPermissionId,
        ...perm,
      } as ButtonPermission & { id: string });
    }
   ...
   * 检查用户是否拥有指定按钮权限
   */
  static hasButtonPermission(
    userPermissions: string[],
    buttonPermissionId: ButtonPermissionId,
   ...
   * 检查用户是否拥有多个按钮权限（全部满足）
   */
  static hasAllButtonPermissions(
    userPermissions: string[],
    buttonPermissionIds: ButtonPermissionId[],
    isAdmin: boolean = false
  ): boolean {
    if (isAdmin) return true;
  ...
  static getDefaultButtonPermissionsByRole(role: string): ButtonPermissionId[] {
    const rolePermissionMap: Record<string, ButtonPermissionId[]> = {
      ...
      // 管理员 - 全部权限
      ADMIN: Object.keys(BUTTON_PERMISSIONS) as ButtonPermissionId[],
      // 所有者 - 全部权限
      OWNER: Object.keys(BUTTON_PERMISSIONS) as ButtonPermissionId[],
    };
