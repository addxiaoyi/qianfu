...
export type EnvironmentIssue = { field: string; message: string };
...
type PaymentProvider = 'xpay' | 'paypro' | 'tpay' | 'hupijiao' | 'creem' | 'qiupay' | 'paypal';
...
const SUPPORTED_PROVIDERS = new Set<PaymentProvider>(['xpay', 'paypro', 'tpay', 'hupijiao', 'creem', 'qiupay', 'paypal']);

const clean = (env: RuntimeEnvInput, field: string): string => env[field]?.trim() || '';

const isStrongSecret = (value: string, minimumLength = 64): boolean => {
  if (value.length < minimumLength || PLACEHOLDER_PATTERN.test(value)) return false;
...
export function validateProductionRuntimeEnv(env: RuntimeEnvInput): ProductionEnvironmentResult {
  ...
  const addWarning = (field: string, message: string) => warnings.push({ field, message });
  const requireValue = (field: string, message = `${field} is required in production`): string => {
    const value = clean(env, field);
    if (!value) addError(field, message);
    return value;
  };
  const validateSecret = (field: string, value: string, minimumLength = 64): void => {
  ...
  const requireSecret = (field: string, minimumLength = 64): string => {
    const value = requireValue(field);
    validateSecret(field, value, minimumLength);
    return value;
  };
  const validateOptionalSecret = (field: string, minimumLength = 64): void => {
    validateSecret(field, clean(env, field), minimumLength);
  };
  const requirePublicHttps = (field: string): URL | null => {
    const value = requireValue(field);
  ...
  const requireServiceUrl = (field: string): URL | null => {
    const value = requireValue(field);
    if (!value) return null;
    const url = parseUrl(value);
  ...
  const personalFilingMode = clean(env, 'PERSONAL_FILING_MODE') === 'true';
  ...
  if (clean(env, 'FORCE_HTTPS') !== 'true') addError('FORCE_HTTPS', 'FORCE_HTTPS must be true in production');
  if (clean(env, 'TRUST_PROXY') !== 'true') addError('TRUST_PROXY', 'TRUST_PROXY must be true behind the production reverse proxy');
  ...
  if (uploadDir && path.isAbsolute(uploadDir)) {
    const publicRoots = [path.resolve(process.cwd(), 'dist'), path.resolve(process.cwd(), 'public')];
    const resolvedUploadDir = path.resolve(uploadDir);
    if (publicRoots.some((root) => resolvedUploadDir === root || resolvedUploadDir.startsWith(`${root}${path.sep}`))) {
      addError('UPLOAD_DIR', 'UPLOAD_DIR must not be inside a public static root');
    }
  }
  ...
  if (githubValues.every(Boolean)) {
    if (!isStrongSecret(githubValues[1], 20)) addError('GITHUB_CLIENT_SECRET', 'GITHUB_CLIENT_SECRET must be a non-placeholder secret');
  ...
  const payproEnabled = clean(env, 'PAYPRO_ENABLED') === 'true';

  if (personalFilingMode) {
    if (clean(env, 'QIANFU_ENABLED') === 'true') {
      addError('QIANFU_ENABLED', 'QIANFU_ENABLED must be false in personal filing mode');
    }
    if (providerRaw) addError('DEFAULT_PAYMENT_UPSTREAM_PROVIDER', 'Payment providers are disabled in personal filing mode');
    if (backupRaw) addError('DEFAULT_PAYMENT_BACKUP_PROVIDER', 'Payment backup providers are disabled in personal filing mode');
    if (redirectHosts.length > 0) addError('VITE_ALLOWED_PAYMENT_REDIRECT_HOSTS', 'Payment redirect hosts are disabled in personal filing mode');
    if (payproEnabled) addError('PAYPRO_ENABLED', 'PAYPRO_ENABLED must be false in personal filing mode');
    if (clean(env, 'PAY_DOMAIN_HOST')) addError('PAY_DOMAIN_HOST', 'Dedicated payment domains are disabled in personal filing mode');
  } else {
    ...
    for (const value of providers) {
      if (!SUPPORTED_PROVIDERS.has(value as PaymentProvider)) {
      ...
      if (provider === 'xpay') {
        ...
        if (!isVmqGateway) requireValue('QIUPAY_PID');
