...
import {
  ...
  buildMarketplacePolicySnapshot,
  buildMarketplaceVersionId,
  hmacEvidenceValue,
  stableJsonStringify,
...
const buildOrderId = (buyerId: number, idempotencyKey: string) => {
  const digest = crypto
    .createHash('sha256')
...
const validateInput = (input: CreateMarketplaceOrderInput): void => {
...
const resolveProductVersion = async (
  tx: Prisma.TransactionClient,
  product: MarketplaceListingRecord,
) => {
