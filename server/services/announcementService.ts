...
import { getR2StorageConfig } from './r2StorageService';
...
const announcementFields = {
  ...
  linkPath: z.preprocess(
    (value) => value === '' || value === undefined ? null : value,
    z.string().trim().regex(/^\/(?!\/)[^\s]*$/, 'Announcement links must be same-site paths').max(200).nullable(),
  ),
...
export function isAllowedAnnouncementImageUrl(value: string): boolean {
...
function validateAnnouncementImages(message: string, context: z.RefinementCtx) {
...
function validateAnnouncementWindow(
  value: { message?: string; startsAt?: string | null; endsAt?: string | null; linkPath?: string | null; linkLabel?: string | null },
  context: z.RefinementCtx,
) {
  if (value.message) validateAnnouncementImages(value.message, context);
...
export function isAnnouncementAllowedInPersonalFiling(
  announcement: Pick<AnnouncementRecord, 'title' | 'message' | 'linkLabel' | 'linkPath'>,
): boolean {
  if (process.env.PERSONAL_FILING_MODE !== 'true') return true;
...
async function assertPublishable(announcement: z.infer<typeof announcementCreateSchema>, adminId: number) {
  if (!isAnnouncementAllowedInPersonalFiling(announcement)) {
    throw new AppError(
      '个人备案模式下，新闻不得宣传支付、钱包、商城、推广或其他交易服务',
      422,
  ...
  if (moderation.passed) return;
  throw new AppError(
    moderation.reason || 'Announcement content did not pass moderation',
    422,
    ErrorCode.VALIDATION_ERROR,
  );
}

function parseStoredAnnouncement(value: string, key: string): AnnouncementRecord | null {
  try {
    const parsed = announcementRecordSchema.safeParse((() => { try { return JSON.parse(value); } catch { return null; } })());
...
export function filterPublicAnnouncements(
  announcements: AnnouncementRecord[],
  now = new Date(),
): AnnouncementRecord[] {
  ...
  return announcements
    .filter((announcement) => {
      if (!isAnnouncementAllowedInPersonalFiling(announcement)) return false;
      if (announcement.status !== 'PUBLISHED') return false;
      if (announcement.startsAt && new Date(announcement.startsAt).getTime() > nowMs) return false;
      if (announcement.endsAt && new Date(announcement.endsAt).getTime() <= nowMs) return false;
      return true;
    })
...
export async function getCurrentAnnouncement(now = new Date()): Promise<AnnouncementRecord | null> {
  return pickActiveAnnouncement(await listAnnouncements(), now);
}

export async function listPublicAnnouncements(now = new Date()): Promise<AnnouncementRecord[]> {
...
export async function updateAnnouncement(
  id: string,
  patch: z.infer<typeof announcementUpdateSchema>,
  adminId: number,
): Promise<{ before: AnnouncementRecord; after: AnnouncementRecord } | null> {
  ...
  const after: AnnouncementRecord = {
    ...before,
    ...merged,
...
export async function deleteAnnouncement(id: string): Promise<AnnouncementRecord | null> {
  const before = await getAnnouncement(id);
  if (!before) return null;
