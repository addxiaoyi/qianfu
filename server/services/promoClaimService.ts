...
const makeRequestNo = (): string => (
  `claim_${Date.now().toString(36)}_${crypto.randomBytes(6).toString('hex')}`
);
...
const createInTransaction = async (
  ...
  input: PendingPromoClaimInput,
  now: Date,
): Promise<PendingPromoClaimResult> => {
...
export const createPendingPromoClaim = async (
  db: PrismaClient,
  input: PendingPromoClaimInput,
): Promise<PendingPromoClaimResult> => {
  for (let attempt = 1; attempt <= MAX_TRANSACTION_ATTEMPTS; attempt += 1) {
    try {
      ...
      return await db.$transaction(
        (tx) => createInTransaction(tx, input, now),
