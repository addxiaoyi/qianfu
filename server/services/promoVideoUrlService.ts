...
const PLATFORM_VIDEO_PATTERNS: Record<string, readonly RegExp[]> = {
...
export const parsePromoVideoUrl = (platformInput: string, rawUrl: string): PromoVideoReference => {
  const platform = normalizePlatform(platformInput);
  const allowedHosts = PLATFORM_HOSTS[platform];
  if (!allowedHosts) throw new AppError('Unsupported promotion platform', 400, ErrorCode.VALIDATION_ERROR);
...
export const getPromoPlatformHosts = (platform: string): readonly string[] => (
  PLATFORM_HOSTS[normalizePlatform(platform)] ?? []
);
