 ...
 * 阿里云 SMS 发送
 */
async function sendViaAliyun(to: string, code: string): Promise<void> {
  ...
  try {
    ...
    if (data.Code !== 'OK') {
      logger.error(
        `[SmsService] 阿里云 SMS 发送失败: Code=${data.Code}, Message=${data.Message}`,
      );
    } else {
 ...
 * 腾讯云 SMS 发送
 */
async function sendViaTencent(to: string, code: string): Promise<void> {
  const config = getConfig();
  ...
  try {
    const response = await axios.post(
      'https://ssr.api.qcloud.com/v2/index.php',
      { ...Object.fromEntries(params), Signature: signature, ...actions },
      { timeout: 10000 },
    );
 ...
 * 发送短信验证码（统一入口）
 */
export async function sendSmsCode(options: SmsSendOptions): Promise<void> {
