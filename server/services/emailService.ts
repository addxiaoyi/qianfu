...
const getFromAddress = () =>
  feishuSmtpFrom || process.env.BREVO_SMTP_FROM || process.env.SMTP_FROM || process.env.EMAIL_FROM || feishuSmtpUser || brevoSmtpUser || process.env.EMAIL_USER;
...
const _sentTimestamps: number[] = [];       // 滑动窗口记录
let _dailyCount = 0;
let _dailyDate = '';

function checkFeishuRateLimit(): { allowed: boolean; reason?: string } {
  if (!useFeishuSmtp) return { allowed: true };
...
const sendViaBrevoApi = async (to: string | string[], subject: string, htmlContent: string) => {
  if (!brevoApiKey) {
...
const sendMailSmart = async (payload: { to: string | string[]; subject: string; html: string; bcc?: string[] }) => {
