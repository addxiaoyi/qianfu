...
const OFFICIAL_BASES: Record<CreemMode, string> = {
...
const normalizeCurrency = (value: unknown): string | undefined => asString(value)?.toUpperCase();

export const normalizeCreemMode = (value: unknown): CreemMode | undefined => {
...
export const resolveCreemApiBaseUrl = (config: CreemRuntimeConfig): string => {
  ...
  if (configured === official) return official;
...
const parseProductMap = (raw: CreemRuntimeConfig['productMap']): Record<string, unknown> => {
  if (!raw) return {};
...
const resolveProductMapEntry = (
  config: CreemRuntimeConfig,
  payment: Pick<CreemInternalPayment, 'plan_id' | 'amount' | 'currency'>,
...
export const resolveCreemCheckoutTerms = (
  config: CreemRuntimeConfig,
  payment: Pick<CreemInternalPayment, 'plan_id' | 'amount' | 'currency'>,
): CreemCheckoutTerms => {
  ...
  const walletCreditAmount = parsePositiveInteger(
    entry.walletCreditAmount ?? entry.wallet_credit_amount ?? payment.amount,
...
export const resolveCreemProductId = (
  config: CreemRuntimeConfig,
  payment: Pick<CreemInternalPayment, 'plan_id' | 'amount' | 'currency'>,
): string => {
  try {
...
const requestCreemJson = async <T>(
  config: CreemRuntimeConfig,
  path: string,
  init: RequestInit,
): Promise<T> => {
...
export const fetchCreemProduct = async (
  config: CreemRuntimeConfig,
  productId: string,
): Promise<CreemProductSnapshot> => {
  const payload = await requestCreemJson<Record<string, unknown>>(
    config,
...
export const createCreemCheckoutSession = async (input: {
  ...
  if (product.price !== terms.checkoutAmount) {
    throw new AppError(
      `Creem product amount mismatch: expected ${terms.checkoutAmount}, got ${product.price}`,
      409,
      ErrorCode.CONFLICT,
    );
  }
  if (product.currency !== terms.checkoutCurrency) {
    throw new AppError(
      `Creem product currency mismatch: expected ${terms.checkoutCurrency}, got ${product.currency}`,
      409,
      ErrorCode.CONFLICT,
    );
  }
  ...
  const payload = await requestCreemJson<Record<string, unknown>>(config, '/v1/checkouts', {
    method: 'POST',
    body: JSON.stringify({
...
const parseWebhookEnvelope = (rawBody: Buffer): CreemWebhookEnvelope => {
  let parsed: unknown;
  try {
    parsed = (() => { try { return JSON.parse(rawBody.toString('utf8')); } catch { return null; } })();
  } catch {
...
const nestedId = (value: unknown): string | undefined =>
  asString(value) || asString(asRecord(value)?.id);

const extractEventRefs = (envelope: CreemWebhookEnvelope): CreemEventRefs => {
...
const findCreemPaymentRecord = async (refs: CreemEventRefs) => {
  if (refs.paymentId) {
  ...
  if (refs.checkoutId) conditions.push({ checkout_id: refs.checkoutId });
  if (refs.orderId) conditions.push({ order_id: refs.orderId });
  if (refs.transactionId) conditions.push({ transaction_id: refs.transactionId });
  if (refs.subscriptionId) conditions.push({ subscription_id: refs.subscriptionId });
  return conditions.length
    ? prisma.creemPaymentRecord.findFirst({ where: { OR: conditions } })
    : null;
};

const assertRecordConsistency = (
  record: Awaited<ReturnType<typeof findCreemPaymentRecord>>,
  refs: CreemEventRefs,
  config: CreemRuntimeConfig,
  eventType: string,
) => {
  if (!record) throw new AppError('Creem payment record not found', 404, ErrorCode.NOT_FOUND);
...
const parseMetadata = (value: string): Record<string, unknown> => {
  try {
    return asRecord((() => { try { return JSON.parse(value); } catch { return null; } })()) || {};
  } catch {
    return {};
  }
};

const roleForPlan = (planId: string): 'VIP' | 'SPONSOR' | undefined => {
  const normalized = planId.toLowerCase();
  if (normalized.includes('vip')) return 'VIP';
  if (normalized.includes('premium') || normalized.includes('pro')) return 'SPONSOR';
  return undefined;
};

const upsertSubscriptionSnapshot = async (
  subscriptionId: string,
  record: NonNullable<Awaited<ReturnType<typeof findCreemPaymentRecord>>>,
  refs: CreemEventRefs,
  envelope: CreemWebhookEnvelope,
) => {
  const object = envelope.object;
...
const grantSubscriptionAccess = async (subscriptionId: string) => {
  const subscription = await prisma.creemSubscription.findUnique({ where: { id: subscriptionId } });
  if (!subscription) return;
  const grantedRole = subscription.granted_role as 'VIP' | 'SPONSOR' | null;
  if (grantedRole) {
    ...
    if (user && !['ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      const permissions = Array.isArray((() => { try { return (() => { try { return JSON.parse(user.permissions || '[]'); } catch { return null; } })(); } catch { return null; } })())
        ? safeJsonParse(user.permissions || '[]', {}) as string[]
        : [];
...
const revokeSubscriptionAccess = async (subscriptionId: string, reason: string) => {
  const subscription = await prisma.creemSubscription.findUnique({ where: { id: subscriptionId } });
  if (!subscription || !subscription.access_active) return;
  await prisma.creemSubscription.update({
    where: { id: subscriptionId },
  ...
  if (otherActive > 0 || !subscription.granted_role) return;
  const user = await prisma.user.findUnique({
    where: { id: subscription.user_id },
    select: { role: true },
  });
  if (!user || ['ADMIN', 'SUPER_ADMIN'].includes(user.role) || user.role !== subscription.granted_role) return;
  await prisma.user.update({
    where: { id: subscription.user_id },
    data: {
...
const reverseWalletCredit = async (
  record: NonNullable<Awaited<ReturnType<typeof findCreemPaymentRecord>>>,
  amount: number,
  eventId: string,
  reason: string,
  ...
  if (!payment || payment.plan_id !== 'custom') return;
...
const revokeOneTimeAccess = async (
  record: NonNullable<Awaited<ReturnType<typeof findCreemPaymentRecord>>>,
  ...
  if (!user || ['ADMIN', 'SUPER_ADMIN'].includes(user.role) || user.role !== grantedRole) return;
  await prisma.user.update({
...
export const processCreemWebhook = async (input: {
  ...
  if (!projectKey) throw new AppError('Creem project key is missing', 404, ErrorCode.NOT_FOUND);
    ...
    try {
      ...
      if (refs.subscriptionId && envelope.eventType.startsWith('subscription.')) {
        ...
        if (ACTIVE_EVENTS.has(envelope.eventType)) {
          ...
          if (periodEnded) await revokeSubscriptionAccess(subscription.id, envelope.eventType);
      ...
      if (envelope.eventType === 'refund.created') {
        ...
        if (fullRefund) {
          if (record.subscription_id) await revokeSubscriptionAccess(record.subscription_id, 'REFUNDED');
      ...
      if (envelope.eventType === 'dispute.created') {
        ...
        if (record.subscription_id) await revokeSubscriptionAccess(record.subscription_id, 'DISPUTED');
...
export const cancelCreemSubscription = async (input: {
  ...
  if (!subscription) throw new AppError('Creem subscription not found', 404, ErrorCode.NOT_FOUND);
...
export const reconcileExpiredCreemSubscriptions = async (): Promise<number> => {
  ...
  if (expired.length) logger.info(`[Creem] Revoked ${expired.length} expired subscription(s)`);
