...
function extractProbeMetrics(status: Record<string, unknown>): {
  online: boolean;
  ...
  const players = status.players as { online?: number; max?: number } | undefined;
  const versionObj = status.version as { name_raw?: string; name?: string } | string | undefined;
  let versionRaw: string | null = null;
  if (typeof versionObj === 'string') versionRaw = versionObj;
  ...
  const latencyMs =
    typeof status.latencyMs === 'number'
      ? status.latencyMs
      : typeof (status as { ping?: number }).ping === 'number'
        ? (status as { ping: number }).ping
        : null;
 ...
 */
export async function tryRecordServerStatusHistory(
  serverId: number,
  ...
  try {
    ...
    logger.warn('[ServerStatusHistory] insert failed', { serverId, error: (e as Error).message });
  }
}

export async function listServerStatusHistory(
  serverId: number,
  since: Date
): Promise<
  Array<{
...
export function aggregateHistoryPoints(
  rows: Array<{ sampled_at: Date; players_online: number | null; online: boolean }>,
  bucketMs: number
): Array<{ t: number; online: number; maxInBucket: number }> {
  if (rows.length === 0) return [];
  const map = new Map<number, { sum: number; count: number; max: number }>();
  for (const r of rows) {
