...
export class ModerationService {
  static async checkText(content: string, userId?: number): Promise<ModerationResult> {
    try {
        ...
        try {
          ...
          await prisma.moderationLog.create({
            data: {
              ...
              content: content.slice(0, 200),
              action: passed ? 'PASSED' : 'REJECTED',
              reason: passed ? null : Object.keys(result.categories).filter(k => result.categories[k]).join(',') + ` (Score: ${Math.max(...Object.values(result.category_scores) as number[])})`
            }
          });
  ...
  static async checkImage(imageUrl: string, userId?: number): Promise<ModerationResult> {
    try {
      const apiKey = await getConfig('MODERATION_API_KEY', true);
      const enabled = (await getConfig('MODERATION_ENABLED')) === 'true';
