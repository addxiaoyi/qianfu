 ...
 */
export const syncServerToMainDB = async (localServerId: number) => {
  try {
 ...
 */
export const syncServerStatusToMainDB = async (serverId: number) => {
  try {
...
export const startPeriodicSync = () => {
  ...
  async function processWithConcurrency<T, R>(
    ...
    for (let i = 0; i < items.length; i += concurrency) {
      ...
      results.push(...batchResults.filter(r => r.status === 'fulfilled').map(r => r.value as R));
    }
    return results;
  }
  ...
  const runSync = async () => {
    try {
      ...
      const unsyncedServers = await localPrisma.$queryRaw<any[]>`
...
export const syncUserToCMS = async (user: { email: string; role: string }) => {
  if (!cmsClient) {
...
export const syncServerToCMS = async (server: Prisma.ServerGetPayload<{}>) => {
  if (!cmsClient) {
