...
export async function submitMarketplaceAppeal(
  appellantId: number,
  input: SubmitMarketplaceAppealInput,
  client: any = prisma,
) {
  let targetId: string;
  if (input.targetType === 'SELLER') {
    ...
    if (!targetId) throw new AppError('Product ID is required', 400, ErrorCode.VALIDATION_ERROR);
    const product = await client.marketplaceProduct.findUnique({
      where: { id: targetId },
      select: { id: true, creator_id: true, listing_status: true },
    });
    if (!product) notFound('Product not found');
    if (product.creator_id !== appellantId) forbidden('Only the product owner can submit this appeal');
  ...
  if (pending) conflict('A pending appeal already exists for this target');
  ...
  try {
    ...
    if (isUniqueConstraintError(error)) conflict('A pending appeal already exists for this target');
    throw error;
  }
}

export async function listMarketplaceAppealsForUser(appellantId: number, client: any = prisma) {
...
export async function listMarketplaceAppeals(status?: string, client: any = prisma) {
...
export async function reviewMarketplaceAppeal(
  appealId: string,
  reviewerId: number,
  input: ReviewMarketplaceAppealInput,
  client: any = prisma,
) {
  const current = await client.marketplaceAppeal.findUnique({ where: { id: appealId } });
  if (!current) notFound('Appeal not found');
  if (current.status !== 'PENDING') conflict('Appeal has already been reviewed');
  ...
  return client.$transaction(async (tx: any) => {
    if (input.decision === 'APPROVED') {
      if (current.target_type === 'SELLER') {
        ...
        if (!Number.isSafeInteger(sellerId) || sellerId <= 0) conflict('Appeal target is invalid');
        ...
        if (result.count !== 1) conflict('Seller is no longer suspended');
        ...
        if (result.count !== 1) conflict('Product is no longer eligible for appeal');
      } else {
