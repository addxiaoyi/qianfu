...
import {
  ...
  createAnnouncement,
  isAnnouncementAllowedInPersonalFiling,
...
export async function listOwnNewsSubmissions(userId: number): Promise<NewsSubmissionView[]> {
...
export async function createNewsSubmission(userId: number, input: NewsSubmissionInput): Promise<NewsSubmissionView> {
...
export async function updateOwnNewsSubmission(
  userId: number,
  id: string,
  input: NewsSubmissionInput,
): Promise<NewsSubmissionView> {
...
export async function listNewsSubmissionsForReview(): Promise<NewsSubmissionView[]> {
  const rows = await prisma.newsSubmission.findMany({
...
export async function approveNewsSubmission(id: string, adminId: number): Promise<NewsSubmissionView> {
  const current = await getSubmission(id);
  if (!current) throw new AppError('投稿不存在', 404, ErrorCode.NOT_FOUND);
...
export async function rejectNewsSubmission(id: string, adminId: number, rejectionReason: string): Promise<NewsSubmissionView> {
  const current = await getSubmission(id);
  if (!current) throw new AppError('投稿不存在', 404, ErrorCode.NOT_FOUND);
  if (current.status !== 'PENDING') {
  ...
  if (!reason) throw new AppError('驳回原因不能为空', 400, ErrorCode.VALIDATION_ERROR);
