...
export class ActivityService {
   ...
   */
  static start(intervalMs: number = 10 * 60 * 1000) {
  ...
  static async updateAllServerActivity() {
    ...
    try {
      ...
      let updatedCount = 0;
      const serverUpdates: { id: number; activity: number }[] = [];
      const statusUpserts: Array<{
        serverId: number;
        online: boolean;
        host: string;
        port: number;
        ...
        await localPrisma.$transaction(async (tx) => {
          ...
          if (serverUpdates.length > 0) {
            const activityUpdatePromises = serverUpdates.map(({ id, activity }) =>
              tx.server.update({ where: { id }, data: { activity } })
            );
   ...
   */
  private static calculateInstantScore(result: {
    status?: {
      online?: boolean;
