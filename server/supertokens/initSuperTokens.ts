...
import type { ProviderInput } from 'supertokens-node/lib/build/recipe/thirdparty/types';
import { middleware, errorHandler } from 'supertokens-node/framework/express';
import { syncPrismaUserFromSuperTokens } from '../services/supertokensPrismaSync';
import { sendSuperTokensPasswordResetEmail } from '../services/emailService';
import { logger } from '../utils/logger';

function emailConfiguredForApp(): boolean {
  return Boolean(process.env.BREVO_SMTP_LOGIN || process.env.SMTP_USER || process.env.EMAIL_USER || process.env.BREVO_API_KEY);
}

function websiteDomain(): string {
            ...
            if (response.status === 'OK') {
              try {
                const email =
                  ...
                  (input.formFields.find((f) => f.id === 'email')?.value as string) ||
                  '';
                await syncPrismaUserFromSuperTokens(response.user.id, email, { emailVerified: false });
              } catch (e) {
    ...
    if (qqConfigured()) {
      providers.push({
        config: {
          ...
          requireEmail: false,
          generateFakeEmail: async ({ thirdPartyUserId, tenantId }: { thirdPartyUserId: string; tenantId: string }) => {
        ...
        override: {
          apis: (oi) => ({
            ...
            signInUpPOST: async (input) => {
              ...
              if (response.status === 'OK') {
                try {
                  const fromApi = response.rawUserInfoFromProvider.fromUserInfoAPI as
                    | Record<string, unknown>
                    | undefined;
                  const provider = input.provider.id;
                  const name =
                    (typeof fromApi?.name === 'string' && fromApi.name) ||
                    (typeof fromApi?.nickname === 'string' && fromApi.nickname) ||
                    (typeof fromApi?.login === 'string' && fromApi.login) ||
                    undefined;
                  const picture =
                    (typeof fromApi?.avatar_url === 'string' && fromApi.avatar_url) ||
