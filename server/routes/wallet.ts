...
import {
  ...
  adminRedeemCodeListQuerySchema,
...
const normalizeRedeemCode = (raw: string) => raw.trim().toUpperCase();

const parseRedeemAmount = (input: string): number => {
...
const generateRandomRedeemCode = (length = 10): string => {
...
router.get('/transactions', async (req, res, next) => {
  try {
    ...
    const sanitizedTransactions = transactions.map(t => {
      ...
      const parsedMetadata = t.metadata ? JSON.parse(t.metadata as string) : null;
...
router.post('/admin/redeem-codes', csrfProtection, async (req, res, next) => {
  try {
    ...
    await logAction(user.id, 'REDEEM_CODE_CREATE', `redeem_code:${code}`, req as any, {
      amount: value.amount,
...
router.patch('/admin/redeem-codes/:code/disable', csrfProtection, async (req, res, next) => {
  try {
    ...
    if (!code) throw new AppError('兑换码不能为空', 400, ErrorCode.BAD_REQUEST);
    const key = `${REDEEM_CODE_PREFIX}${code}`;

    const cfg = await prisma.systemConfig.findUnique({ where: { key } });
    if (!cfg) throw new AppError('兑换码不存在', 404, ErrorCode.NOT_FOUND);
    ...
    await logAction(user.id, 'REDEEM_CODE_DISABLE', `redeem_code:${code}`, req as any, {
      code,
...
router.delete('/admin/redeem-codes/:code', csrfProtection, async (req, res, next) => {
  try {
    ...
    const existed = await prisma.systemConfig.findUnique({ where: { key } });
    if (!existed) throw new AppError('兑换码不存在', 404, ErrorCode.NOT_FOUND);
    ...
    await logAction(user.id, 'REDEEM_CODE_DELETE', `redeem_code:${code}`, req as any, {
      code,
      snapshot,
    });
