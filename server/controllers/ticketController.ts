...
export const getTickets = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const where: any = req.isAdmin ? {} : { user_id: req.user!.id };
        ...
        const normalizedSortField = resolveSortField(
            sortBy,
            ['updated_at', 'created_at', 'priority', 'status'] as const,
            'updated_at',
        );
 ...
 */
export const addMessage = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const idValidation = idParamSchema.safeParse(req.params);
        if (!idValidation.success) {
