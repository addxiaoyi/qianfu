...
import {
  PLAN_PRICES_FEN,
  buildProjectScopedPaymentId,
  completeExternalPayment,
  createExternalPaymentByProvider,
  getPaymentProjectConfig,
  normalizePlanId,
  parseProjectKeyFromPaymentId,
} from './paymentController';

const PAYMENT_PROJECT_CONFIG_PREFIX = 'payment_project:';
const SUPPORTED_UPSTREAM_PROVIDER_ORDER = ['creem', 'qiupay', 'xpay', 'tpay', 'hupijiao', 'paypro'] as const;
const SUPPORTED_UPSTREAM_PROVIDERS = new Set<string>(SUPPORTED_UPSTREAM_PROVIDER_ORDER);
...
type XpayAdminTenant = {
  id: number;
  tenantKey: string;
  displayName?: string;
  callbackUrl?: string;
  status?: number;
  paymentMethods?: XpayAdminTenantMethod[];
};

const extractXpayEnvelopePayload = <T>(payload: XpayAdminEnvelope<T>): T | undefined =>
  payload.result ?? payload.data;

const resolveXpayAdminBaseUrl = (config: Record<string, unknown>): string => {
  const candidates = [
    XPAY_ADMIN_BASE_URL,
...
const resolveXpayPublicBaseUrl = (config: Record<string, unknown>, fallbackHost?: string): string => {
  const candidates = [
    String(config.xpayGatewayBaseUrl || '').trim(),
    process.env.XPAY_PUBLIC_URL?.trim() || '',
    process.env.XPAY_GATEWAY_BASE_URL?.trim() || '',
...
const buildTenantCallbackUrl = (req: Request): string => {
...
const loginXpayAdmin = async (baseUrl: string): Promise<{ token: string; profile: Record<string, unknown> | null }> => {
  ...
  const loginPayload = await loginResponse.json().catch(() => null) as XpayAdminEnvelope<{ token?: string }> | null;
  const loginResult = loginPayload ? extractXpayEnvelopePayload(loginPayload) : undefined;
  const token = loginResult?.token;

  if (!loginResponse.ok || !token) {
    throw new AppError(
      `XPay admin login failed: ${loginPayload?.message || loginPayload?.msg || `HTTP ${loginResponse.status}`}`,
      502,
  ...
  const profilePayload = await profileResponse.json().catch(() => null) as XpayAdminEnvelope<Record<string, unknown>> | null;
...
const xpayAdminJson = async <T>(
  ...
  const payload = await response.json().catch(() => null) as XpayAdminEnvelope<T> | null;
  const result = payload ? extractXpayEnvelopePayload(payload) : undefined;
  if (!response.ok) {
    throw new AppError(
      `XPay admin request failed: ${payload?.message || payload?.msg || `HTTP ${response.status}`}`,
...
const findXpayTenant = async (
  baseUrl: string,
  token: string,
  tenantKey: string,
): Promise<XpayAdminTenant | null> => {
  const tenants = await xpayAdminJson<XpayAdminTenant[]>(baseUrl, token, '/admin/tenants');
  return (tenants || []).find((item) => item?.tenantKey === tenantKey) || null;
};

const validateProviderConfig = (config: any, provider: string, label: 'primary' | 'backup') => {
  ...
  if (provider === 'xpay') {
    ...
    const tenantGatewayReady =
      hasText(config.xpayGatewayBaseUrl) &&
    ...
    if (tenantGatewayFieldsPresent && !tenantGatewayReady) {
      throw new AppError(
        `${prefix}XPay tenant gateway requires xpayGatewayBaseUrl + xpayTenantKey + xpayToken + xpayTenantCallbackSecret`,
        400,
...
const resolveXpayMode = (config: any): 'tenant-gateway' | 'legacy' | 'none' => {
...
const buildProviderReadiness = (config: any) => {
...
const buildProjectStatus = (config: Record<string, unknown>) => {
...
const normalizeProjectKey = (raw: unknown): string => {
...
export const listPaymentProjects = async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
export const upsertPaymentProject = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const deletePaymentProject = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
...
export const getPaymentProjectDiagnostics = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    ...
    const tests = diagnostics.tests as Array<Record<string, unknown>>;
      ...
      if (xpayMode === 'tenant-gateway' && config.xpayGatewayBaseUrl && config.xpayTenantKey && config.xpayToken) {
        ...
        try {
          const response = await fetch(
            `${config.xpayGatewayBaseUrl.replace(/\/+$/, '')}/open/tenants/${encodeURIComponent(config.xpayTenantKey)}`,
            {
              method: 'GET',
              headers: {
    ...
    if (config.upstreamProvider === 'creem' || config.backupUpstreamProvider === 'creem') {
      tests.push({
        name: 'creemProductId',
        ok: hasText((config as any).creemProductId),
        detail: String((config as any).creemProductId || 'missing'),
      });
      tests.push({
        name: 'creemWebhookSecret',
        ok: hasText((config as any).creemWebhookSecret),
        detail: hasText((config as any).creemWebhookSecret) ? 'configured' : 'missing',
      });
    }
    ...
    if (config.upstreamProvider === 'qiupay' || config.backupUpstreamProvider === 'qiupay') {
      tests.push({
        name: 'qiupayPid',
        ok: hasText((config as any).qiupayPid),
        detail: String((config as any).qiupayPid || 'missing'),
      });
      tests.push({
        name: 'qiupayNotifyUrl',
        ok: hasText((config as any).qiupayNotifyUrl) || true,
        detail: String((config as any).qiupayNotifyUrl || '/api/v1/payment/qiupay/notify'),
      });
    }
...
export const getPaymentProjectXpayTenant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    const config = await getPaymentProjectConfig(projectKey);
    const adminBaseUrl = resolveXpayAdminBaseUrl(config as unknown as Record<string, unknown>);
    ...
    return sendSuccess(res, {
      ...
      resolved: {
        xpayMode: buildProjectStatus(config as unknown as Record<string, unknown>).xpayMode,
        tokenConfigured: hasText(config.xpayToken),
        gatewayNotifyConfigured: hasText((config as any).xpayGatewayNotifySecret),
        tenantCallbackConfigured: hasText((config as any).xpayTenantCallbackSecret),
...
export const syncPaymentProjectXpayTenant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    const config = await getPaymentProjectConfig(projectKey);
    const adminBaseUrl = resolveXpayAdminBaseUrl(config as unknown as Record<string, unknown>);
    const publicBaseUrl = resolveXpayPublicBaseUrl(
    ...
    let tenant: XpayAdminTenant | null = null;
    let accessToken = '';
    let callbackSecret = '';
    let action: 'create' | 'rotate' = 'create';

    if (existingTenant) {
      action = 'rotate';
      await xpayAdminJson<Map<string, unknown>>(adminBaseUrl, token, `/admin/tenants/${existingTenant.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const rotated = await xpayAdminJson<{ accessToken: string; callbackSecret: string }>(
        adminBaseUrl,
        token,
        `/admin/tenants/${existingTenant.id}/rotate-secrets`,
        {
          method: 'POST',
        },
      );
      ...
      const created = await xpayAdminJson<{
        tenant: XpayAdminTenant;
        accessToken: string;
        callbackSecret: string;
      }>(adminBaseUrl, token, '/admin/tenants', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
export const uploadPaymentProjectXpayTenantQr = async (req: Request & { file?: Express.Multer.File }, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    ...
    const payload = await response.json().catch(() => null) as XpayAdminEnvelope<Record<string, unknown>> | null;
    const result = payload ? extractXpayEnvelopePayload(payload) : undefined;
    if (!response.ok) {
      throw new AppError(
        `XPay QR upload failed: ${payload?.message || payload?.msg || `HTTP ${response.status}`}`,
        response.status >= 400 && response.status < 600 ? response.status : 502,
        ErrorCode.SERVICE_UNAVAILABLE,
      );
    }
...
export const createPaymentProjectTestOrder = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    const body = req.body && typeof req.body === 'object' ? req.body as Record<string, unknown> : {};
    ...
    try {
      const provider = providerOverride && SUPPORTED_UPSTREAM_PROVIDERS.has(providerOverride)
        ...
        : projectConfig.upstreamProvider as any;
      const providerResult = await createExternalPaymentByProvider(req, payment, projectConfig as any, amount, paymentMethod as 'alipay' | 'wechat', provider);
      return sendSuccess(res, {
        projectKey,
...
export const getPaymentProjectOrder = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
...
export const simulatePaymentProjectOrderSuccess = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const projectKey = normalizeProjectKey(req.params.projectKey);
    const orderId = String(req.params.orderId || '').trim();
    if (!orderId) {
