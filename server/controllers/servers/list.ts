 ...
 */
export const listAllServers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cached = await redisService.get<{ servers: unknown[], total: number }>(cacheKey);
    ...
    if (keyword) {
      andConditions.push({
        OR: buildKeywordOrConditions(
          ['name', 'name_en', 'summary', 'summary_en', 'tags', 'ip'],
          keyword,
          fuzzy,
        ),
      });
    ...
    const normalizedSortBy = resolveSortField(
      sortBy,
      ['activity', 'updated', 'created', 'players', 'name'] as const,
      'activity',
    );
    const normalizedSortOrder = resolveSortOrder(sortOrder, 'desc');

    const safeOrderBy: Prisma.ServerOrderByWithRelationInput[] = (() => {
      if (!normalizedSortBy) return SERVER_ORDER_BY;
