...
import {
  serverSchema, idParamSchema,
  validateHost, isSafeHostname
 ...
 */
function effectiveServerCap(user: any) {
 ...
 * Create a new server
 */
export const createServer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const user = req.user;
    ...
    let cleanIp: string | undefined = undefined;
    if (ip) {
 ...
 */
export const updateServer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const {
      ...
      content_html, ip, group_number, tags, link,
      platform, category, online_mode, supported_versions, network_env,
    ...
    // IP/Host Normalization & Duplicate Check
    let finalIp: string = existingServer.ip ?? '';
    if (ip) {
 ...
 * Delete a server
 */
export const deleteServer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const idValidation = idParamSchema.safeParse(req.params);
    if (!idValidation.success) {
      throw new AppError('Invalid server ID', 400, ErrorCode.VALIDATION_ERROR, false, idValidation.error.issues);
