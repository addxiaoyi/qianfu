...
import { buildFullAiSystemPrompt } from '../config/aiProductKnowledge';
...
const getLanguage = (req: Request): 'en' | 'zh' => {
  const bodyLang = req.body?.language;
  if (bodyLang === 'en' || bodyLang === 'zh') return bodyLang;
  return 'zh';
};

const checkRateLimit = async (userId: number | undefined, ip: string, today: string) => {
  if (userId) {
    const dailyLimitKey = `ai:daily:${userId}:${today}`;
    const dailyCount = await redisService.incr(dailyLimitKey, 24 * 60 * 60);
    if (dailyCount > 100) throw new AppError('Daily AI limit reached (100/day)', 429, ErrorCode.LIMIT_EXCEEDED);
  } else {
    const guestLimitKey = `ai:daily:guest:${ip}:${today}`;
    const guestCount = await redisService.incr(guestLimitKey, 24 * 60 * 60);
    if (guestCount > 20) throw new AppError('Guest limit reached (20/day)', 429, ErrorCode.LIMIT_EXCEEDED);
  }
};

const detectJailbreak = (message: string, userId: number | undefined, language: string, res: Response) => {
...
const callZhipuAPI = async (messages: any[], language: string) => {
...
export const chat = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!ZHIPU_API_KEY) throw new AppError('Service unavailable', 503, ErrorCode.INTERNAL_ERROR);

    const activeIds = clientMeta?.activeIntegrationIds;
    const messages = [
      {
        role: 'system',
        content: buildFullAiSystemPrompt(
          language || 'zh',
          Array.isArray(activeIds) && activeIds.length > 0 ? activeIds : null
        ),
      },
    ];
    if (req.user) messages.push({ role: 'system', content: `Logged-in user (server): ${req.user.username}` });
    ...
    if ('msg' in result) return sendSuccess(res, { reply: result.msg });

    return sendSuccess(res, { reply: result.reply });
  } catch (error) {
    logger.error('[AI Chat] Error:', error);
    const language = getLanguage(req);
    return next(error instanceof Error ? error : new AppError(
      language === 'en' ? 'AI offline.' : 'AI 助手离线。',
      503,
