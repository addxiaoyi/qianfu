...
import { sendDetailResponse, sendSuccess } from '../utils/response';
...
const parsePagination = (query: AuthRequest['query']) => {
  const page = Math.max(1, Number(query.page ?? 1));
  const limit = Math.min(50, Math.max(1, Number(query.limit ?? 20)));
  return { page, limit, skip: (page - 1) * limit };
};

export const getUserPromoTask = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction,
): Promise<unknown> => {
...
export const getAdminPromoTask = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction,
): Promise<unknown> => {
...
export const listAdminPromoClaims = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction,
): Promise<unknown> => {
