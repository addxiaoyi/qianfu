...
export const getAllTeamMembers = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getAllAllianceGroups = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const validation = staticDataQuerySchema.safeParse(req.query);
    if (!validation.success) {
      throw new AppError('Invalid query parameters', 400, ErrorCode.VALIDATION_ERROR);
...
export const getAllResourceLinks = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const validation = staticDataQuerySchema.safeParse(req.query);
    if (!validation.success) {
      throw new AppError('Invalid query parameters', 400, ErrorCode.VALIDATION_ERROR);
