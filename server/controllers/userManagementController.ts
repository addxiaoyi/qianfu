...
export const listUsers = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const normalizedSortField = resolveSortField(
      sortBy,
      ['created_at', 'last_login_at', 'username', 'email'] as const,
      'created_at',
    );
    ...
    const usersWithRoleInfo = users.map(u => ({
      ...
      roleInfo: PermissionGroupManager.getGroup(u.role as PermissionGroup)
    }));
...
export const updateUserRole = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const validation = PermissionGroupManager.validateGroupAssignment(
      ...
      adminPermissions,
      adminUser.role as PermissionGroup
    );
    ...
    const groupInfo = PermissionGroupManager.getGroup(role as PermissionGroup);
    ...
    await logDataChange(adminUser.id, 'UPDATE_USER_ROLE', `user_${updatedUser.id}`, req as any, targetUser, updatedUser);
    ...
    const stats = await withCache(USER_STATS_CACHE_KEY, async () => {
      ...
      return {
        ...
        roleStats: roleStats.map(stat => ({
          ...
          roleInfo: PermissionGroupManager.getGroup(stat.role as PermissionGroup)
        }))
      };
...
export const getAvailableRoles = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const user = req.user;
    if (!user || !user.permissions) {
      throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
    ...
    const roles = Object.entries(groups)
      .filter(([key]) => {
        ...
        return PermissionGroupManager.validateGroupAssignment(
          'USER' as PermissionGroup,
          key as PermissionGroup,
          userPermissions,
          user.role as PermissionGroup
        ).valid;
      })
      .map(([key, group]) => ({
        role: key,
