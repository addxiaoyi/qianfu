...
export const getReports = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const isAdmin = (req as AuthRequest).isAdmin || user.role === 'ADMIN' || user.role === 'MODERATOR';
    ...
    if (target_type) where.target_type = String(target_type);
...
export const getReport = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!report) throw new AppError('Report not found', 404, ErrorCode.NOT_FOUND);
...
export const batchUpdateReportStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const user = (req as AuthRequest).user;
        if (!user) throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
