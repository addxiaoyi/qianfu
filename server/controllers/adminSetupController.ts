...
export const setupSoleAdmin = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const getAdminStatus = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const admins = await prisma.user.findMany({
      where: { role: 'ADMIN' },
      select: { 
