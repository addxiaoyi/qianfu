...
const toIsoDate = (value: unknown, fallback = new Date().toISOString()) => {
  if (!value) return fallback;
  const date = new Date(String(value));
  return Number.isNaN(date.getTime()) ? fallback : date.toISOString();
};

const buildPublicSitemap = async (): Promise<string> => {
  const [servers, products, resources, teamMembers] = await Promise.all([
    ...
    prisma.resourceLink.findMany({
      ...
      orderBy: { updated_at: 'desc' },
      take: 200,
    }).catch(() => [] as Array<{ updated_at: Date | string | null }>),
    prisma.teamMember.findMany({
      select: { id: true, updated_at: true, created_at: true },
      orderBy: { updated_at: 'desc' },
      take: 200,
    }).catch(() => [] as Array<{ id: number; updated_at: Date | string | null; created_at: Date | string | null }>),
  ]);
...
export const getLlmsTxt = (_req: Request, res: Response) => {
  res.type('text/plain; charset=utf-8').send(buildLlms());
};

export const getSitemapXml = async (_req: Request, res: Response, next: NextFunction) => {
  try {
