...
import { fromBufferPromise, open as openArchive, validateFileName } from 'yauzl';
...
interface InspectionState extends ArchiveInspectionSummary {
...
function normalizeEntryName(fileName: string): string {
  if (!fileName || fileName.includes('\0')) throw archiveError('empty or null-containing entry name');
  ...
  if (validationError) throw archiveError(validationError);
...
function assertRegularEntry(entry: Entry, normalizedName: string): void {
  const fileType = unixFileType(entry);
  const isDirectory = entry.fileName.endsWith('/');
  if (fileType === 0o120000) throw archiveError(`symbolic links are not allowed: ${normalizedName}`);
...
function isNestedArchive(fileName: string): boolean {
  const lower = fileName.toLowerCase();
  return ARCHIVE_SUFFIXES.some((suffix) => lower.endsWith(suffix));
}

async function readStreamWithLimit(stream: NodeJS.ReadableStream, maxBytes: number): Promise<Buffer> {
  const chunks: Buffer[] = [];
  let total = 0;
  for await (const chunk of stream as AsyncIterable<Buffer | string>) {
    const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
    total += buffer.length;
    if (total > maxBytes) throw archiveError('nested archive exceeds inspection buffer limit');
    chunks.push(buffer);
  }
  return Buffer.concat(chunks, total);
}

async function inspectLevel(
  source: Buffer | { filePath: string },
  depth: number,
  scope: string,
  state: InspectionState,
): Promise<void> {
  if (depth > ARCHIVE_LIMITS.maxNestingDepth) throw archiveError('nested archive depth exceeded');
  state.maxDepth = Math.max(state.maxDepth, depth);

  const options: Options = {
    autoClose: !Buffer.isBuffer(source),
    lazyEntries: true,
    decodeStrings: true,
    validateEntrySizes: true,
    strictFileNames: true,
  };
  const zipFile = await (Buffer.isBuffer(source)
    ? fromBufferPromise(source, options)
    : openArchivePromise(source.filePath, options)
  ).catch((error: unknown) => {
  ...
  try {
    if (zipFile.entryCount > ARCHIVE_LIMITS.maxEntries) throw archiveError('entry count exceeded');

    for await (const entry of zipFile.eachEntry()) {
      state.entryCount += 1;
      if (state.entryCount > ARCHIVE_LIMITS.maxEntries) throw archiveError('entry count exceeded');
      if (entry.isEncrypted()) throw archiveError(`encrypted entries are not allowed: ${entry.fileName}`);
      ...
      if (state.seenPaths.has(identity)) throw archiveError(`duplicate entry path: ${normalizedName}`);
      ...
      if (entry.uncompressedSize > 1024 * 1024) {
        if (entry.compressedSize <= 0) throw archiveError(`invalid compressed size: ${normalizedName}`);
    ...
    if (error instanceof Error && error.message.startsWith('Unsafe archive:')) throw error;
...
function openArchivePromise(filePath: string, options: Options): Promise<ZipFile> {
  return new Promise((resolve, reject) => {
    openArchive(filePath, options, (error, zipFile) => {
      if (error || !zipFile) {
        reject(error || new Error('invalid ZIP structure'));
        return;
      }
      resolve(zipFile);
    });
...
export async function inspectArchiveBuffer(buffer: Buffer): Promise<ArchiveInspectionSummary> {
  if (!Buffer.isBuffer(buffer) || buffer.length < 22) throw archiveError('file is not a valid ZIP archive');
...
export async function inspectArchiveFile(filePath: string): Promise<ArchiveInspectionSummary> {
  const state: InspectionState = {
    entryCount: 0,
    totalExpandedBytes: 0,
    nestedArchiveCount: 0,
  ...
  await inspectLevel({ filePath }, 0, 'root', state);
  return {
    entryCount: state.entryCount,
    totalExpandedBytes: state.totalExpandedBytes,
    nestedArchiveCount: state.nestedArchiveCount,
