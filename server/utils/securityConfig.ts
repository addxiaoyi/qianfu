import { AppError, ErrorCode } from './errors';

const MIN_JWT_SECRET_LENGTH = 32;

let jwtSecretCache: string | null = null;
let trustedHostCache: Set<string> | null = null;

const normalizeHost = (value: string): string => {
  return value.trim().toLowerCase().replace(/\.$/, '');
};

const parseHostList = (raw: string): Set<string> => {
  return new Set(
    raw
      .split(',')
      .map((entry) => normalizeHost(entry))
      .filter(Boolean),
  );
};

const deriveDefaultTrustedHosts = (): Set<string> => {
  const hosts = new Set<string>();
  const candidates = [
    process.env.API_PUBLIC_URL,
    process.env.FRONTEND_URL,
    process.env.CORS_ORIGIN,
  ];

  for (const candidate of candidates) {
    if (!candidate?.trim()) continue;
    try {
      hosts.add(normalizeHost(new URL(candidate).host));
    } catch {
      continue;
    }
  }

  return hosts;
};

export const getJwtSecret = (): string => {
  if (jwtSecretCache) {
    return jwtSecretCache;
  }

  const raw = process.env.JWT_SECRET?.trim();
  if (!raw || raw.length < MIN_JWT_SECRET_LENGTH) {
    throw new AppError(
      `JWT secret is missing or too short (minimum ${MIN_JWT_SECRET_LENGTH} characters required)`,
      500,
      ErrorCode.SERVICE_UNAVAILABLE,
    );
  }

  jwtSecretCache = raw;
  return raw;
};

export const getTrustedRedirectHosts = (): Set<string> => {
  if (trustedHostCache) {
    return trustedHostCache;
  }

  const configured = process.env.TRUSTED_REDIRECT_HOSTS?.trim();
  if (configured) {
    trustedHostCache = parseHostList(configured);
    return trustedHostCache;
  }

  trustedHostCache = deriveDefaultTrustedHosts();
  return trustedHostCache;
};

export const isTrustedHost = (host: string): boolean => {
  return getTrustedRedirectHosts().has(normalizeHost(host));
};
