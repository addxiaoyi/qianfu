import { PERMISSION_GROUPS, type PermissionGroup } from '../config/permissionGroups';
import { parseJsonArray } from './jsonField';
...
const KNOWN_USER_PERMISSION_SET = new Set<string>(KNOWN_USER_PERMISSIONS);
...
const SUPPLEMENTAL_STORED_PERMISSIONS = new Set<KnownUserPermission>([
  'manage_stats',
  'port5555_access',
  'sponsor_badge',
  'priority_support',
...
const PRIVILEGED_PERMISSION_ROLES: Partial<Record<KnownUserPermission, ReadonlySet<string>>> = {
...
export function isKnownUserPermission(value: unknown): value is KnownUserPermission {
...
function getRolePermissions(role: unknown): readonly KnownUserPermission[] | null {
  const normalizedRole = typeof role === 'string' ? role.trim().toUpperCase() : '';
  if (!Object.prototype.hasOwnProperty.call(PERMISSION_GROUPS, normalizedRole)) {
    return null;
  }
  return PERMISSION_GROUPS[normalizedRole as PermissionGroup].permissions as readonly KnownUserPermission[];
}

function isPermissionAllowedForRole(permission: KnownUserPermission, role: unknown): boolean {
...
export function parseAuthorizedPermissions(value: unknown, role: unknown): KnownUserPermission[] {
...
export function hasAuthorizedPermission(
  role: unknown,
