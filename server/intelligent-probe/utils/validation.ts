 ...
 */
export const validateHost = (host: string): ValidationError | null => {
 ...
 * @returns ValidationError object if invalid, otherwise null
 */
export const validateUrl = (urlString: string): ValidationError | null => {
...
export const validate = <T>(schema: ZodSchema<T>, data: any): T => {
  try {
    ...
    if (error instanceof ZodError) {
      ...
      throw new Error(`Validation failed: ${error.issues.map((e: ZodIssue) => e.message).join(', ')}`);
    }
    throw error;
  }
};
