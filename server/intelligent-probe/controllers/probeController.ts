...
export const getServerStatus = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const validationError = validateHost(host as string);
    if (validationError) {
      throw new Error(validationError.message);
    }

    // Call service layer to get server status
    const serverStatus = await getMinecraftServerStatus(
      host as string,
      bedrock === 'true'
    );
 ...
 * @param next Error passing middleware
 */
export const getMultipleServerStatusHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const server of servers) {
      ...
      if (err) throw new Error(`Invalid host ${server.host}: ${err.message}`);
    }
