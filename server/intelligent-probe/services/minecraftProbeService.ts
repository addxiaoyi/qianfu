 ...
 */
const getMcsrvstatStatus = async (hostname: string, port: number, bedrock: boolean) => {
  try {
...
/**
 * Resolves hostname and validates IP safety
 */
type DnsLookupResult = string | { address: string; family: number };

const resolveSafeAddress = async (hostname: string): Promise<string> => {
  try {
    const result = (await lookup(hostname)) as DnsLookupResult;
 ...
 */
const resolveTargetInfo = async (hostname: string, port: number, bedrock: boolean) => {
 ...
 */
const performDirectProbe = async (targetAddress: string, targetPort: number, bedrock: boolean) => {
 ...
 */
export const getMinecraftServerStatus = async (host: string, bedrock: boolean) => {
  ...
  try {
    ...
    try {
      ...
      if (!result) throw new Error('Minecraft server probe returned no data.');
 ...
 */
export const getMultipleServerStatus = async (
  servers: ServerProbeRequest[],
  maxConcurrent: number = MAX_CONCURRENT_PROBES
  ...
  const results: ServerProbeResult[] = new Array(servers.length);
  let currentIndex = 0;
