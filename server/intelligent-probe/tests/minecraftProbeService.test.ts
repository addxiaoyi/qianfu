...
vi.mock('node:dns', () => {
  return {
    default: {
      lookup: (hostname: string, callback: any) => callback(null, { address: '8.8.8.8' }),
      resolveSrv: (hostname: string, callback: any) => callback(null, [])
    },
    lookup: (hostname: string, callback: any) => callback(null, { address: '8.8.8.8' }),
    resolveSrv: (hostname: string, callback: any) => callback(null, [])
  };
...
describe('minecraftProbeService', () => {
  ...
  it('should successfully probe a Java server and return formatted status', async () => {
    ...
    (mcutil.status as any).mockResolvedValue({
  ...
  it('should successfully probe a Bedrock server', async () => {
    (mcutil.statusBedrock as any).mockResolvedValue({
  ...
  it('should throw a custom error if all retries fail', async () => {
    ...
    (mcutil.status as any).mockRejectedValue(new Error(errorMessage));
