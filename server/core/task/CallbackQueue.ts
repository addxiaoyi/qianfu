...
import { assertSafeOutboundCallbackUrl } from './callbackOutboundPolicy';
...
export class CallbackQueue {
  private static instance: CallbackQueue;
  private isProcessing = false;
  private processorInterval: NodeJS.Timeout | null = null;
  private queue: Map<string, CallbackTask> = new Map();
  private retryQueue: Map<string, CallbackTask> = new Map();
  ...
  async addTask(url: string, payload: Record<string, any>): Promise<string> {
  ...
  async retryTask(taskId: string): Promise<void> {
    const taskJson = await redisService.get<string>(`${CALLBACK_QUEUE_KEY}:${taskId}`);
    if (!taskJson) {
      logger.warn(`[CallbackQueue] Task ${taskId} not found for retry`);
      return;
    }

    const task: CallbackTask = JSON.parse(taskJson);
    task.attempts++;
  ...
  private async moveToDeadLetter(task: CallbackTask): Promise<void> {
  ...
  async executeCallback(task: CallbackTask): Promise<CallbackResult> {
    try {
      ...
      if (response.ok) {
        return { success: true, response: await response.json(), shouldRetry: false };
      }
  ...
  private startProcessor(): void {
    ...
    this.processorInterval = setInterval(() => {
      this.processQueue().catch((err: unknown) => logger.error('CallbackQueue process failed', { error: err }));
    }, 30000);
