...
export class SignatureUtil {
  ...
  constructor(appId: string, secretKey: string, timeoutSeconds: number = DEFAULT_TIMEOUT_SECONDS) {
    this.appId = appId;
  ...
  static isTimestampExpired(timestamp: string, timeoutSeconds: number = DEFAULT_TIMEOUT_SECONDS): boolean {
    const now = Math.floor(Date.now() / 1000);
    const requestTime = parseInt(timestamp, 10);
    if (isNaN(requestTime)) return true;
...
export const qianfuConfig: QianFuConfig = {
