...
interface MarketplaceProduct {
  id: string;
  title: string;
  category: 'map' | 'plugin' | 'mod' | 'modpack' | 'resource_pack' | 'template';
  description: string;
  price: number;
  sales: number;
  rating: number;
...
interface MarketplaceShopConfig {
  bannerUrl: string;
  avatarUrl: string;
...
interface MarketplaceShopMetric {
  visits: number;
...
interface MarketplaceShopConfigVersion {
  id: string;
  config: MarketplaceShopConfig;
  createdAt: string;
}

const makeMarketplaceId = (prefix: string) => `${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;

const defaultMarketplaceProducts: MarketplaceProduct[] = [
...
const defaultMarketplaceOrders: MarketplaceOrder[] = [];

let marketplaceProducts: MarketplaceProduct[] = [...defaultMarketplaceProducts];
let marketplaceOrders: MarketplaceOrder[] = [...defaultMarketplaceOrders];

const defaultShopConfig: MarketplaceShopConfig = {
...
let marketplaceShopConfig: MarketplaceShopConfig = { ...defaultShopConfig };
let marketplaceShopMetrics: MarketplaceShopMetric = {
  visits: 0,
  announcementClicks: 0,
  featuredClicks: 0,
  updatedAt: new Date().toISOString(),
};
let marketplaceShopVersions: MarketplaceShopConfigVersion[] = [];
...
const loadJsonStorage = async <T,>(key: string, fallback: T): Promise<T> => {
  const cached = await redisService.get<string>(key).catch(() => null);
...
const saveJsonStorage = async (key: string, value: unknown) => {
...
const loadMarketplace = async () => {
  const [products, orders, storedConfig, storedMetrics, storedVersions] = await Promise.all([
    ...
    loadJsonStorage<MarketplaceShopConfig | null>(SHOP_CONFIG_STORAGE_KEY, null),
    loadJsonStorage<MarketplaceShopMetric | null>(SHOP_METRICS_STORAGE_KEY, null),
    loadJsonStorage<MarketplaceShopConfigVersion[] | null>(SHOP_VERSIONS_STORAGE_KEY, null),
  ]);

  if (storedConfig) marketplaceShopConfig = { ...defaultShopConfig, ...storedConfig };
  if (storedMetrics) marketplaceShopMetrics = storedMetrics;
  if (storedVersions) marketplaceShopVersions = storedVersions;

  marketplaceProducts = products.map((product) => ({
    id: product.id,
    title: product.title,
    category: product.category as MarketplaceProduct['category'],
    description: product.description,
    price: product.price,
    sales: product.sales,
    rating: product.rating,
  ...
  marketplaceOrders = orders.map((order) => ({
    ...
    status: order.status as MarketplaceOrder['status'],
    paymentStatus: order.payment_status as MarketplaceOrder['paymentStatus'],
    fulfillmentStatus: order.fulfillment_status as MarketplaceOrder['fulfillmentStatus'],
...
const mapFavoriteIds = (product: MarketplaceProduct, currentUserId?: number | null) => ({
  ...product,
...
const snapshotShopConfig = (config: MarketplaceShopConfig): MarketplaceShopConfigVersion => ({
  id: makeMarketplaceId('shopver'),
  config: { ...config },
  createdAt: new Date().toISOString(),
});

const themePresets: Record<MarketplaceShopConfig['theme'], Partial<MarketplaceShopConfig>> = {
  default: {},
  tech: {
...
const QIANFU_NOTIFY_REPLAY_TTL_SECONDS = parseNonNegativeIntegerEnv(
  'QIANFU_NOTIFY_REPLAY_TTL_SECONDS',
  PAYMENT_NOTIFY_REPLAY_TTL_SECONDS,
);
const QIANFU_NOTIFY_IP_ALLOWLIST = resolveNotifyIpAllowlist(
  process.env.QIANFU_NOTIFY_IP_ALLOWLIST,
  process.env.PAYMENT_NOTIFY_IP_ALLOWLIST || qianfuConfig.whitelist?.join(','),
);
...
router.post('/payment/create', async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!qianfuConfig.enabled) {
      return sendError(res, 'QianFu integration is disabled', 400);
    }

    const { outTradeNo, amount, payType, subject } = req.body as CreatePaymentRequest;
...
router.get('/payment/query/:orderId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!qianfuConfig.enabled) {
      return sendError(res, 'QianFu integration is disabled', 400);
    }
...
router.post('/payment/close/:orderId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!qianfuConfig.enabled) {
      return sendError(res, 'QianFu integration is disabled', 400);
    }
...
router.post('/xpay/notify', async (req: Request, res: Response, next: NextFunction) => {
  let replayKey: string | null = null;
  try {
    const notifyData = req.body as QianFuPaymentNotify;
...
router.get('/reconciliation/daily', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/reconciliation/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { startDate, endDate } = req.query;

    const start = (startDate as string) || new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
...
router.get('/reconciliation/exceptions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { startDate, endDate } = req.query;

    const start = (startDate as string) || new Date(Date.now() - 86400000).toISOString().split('T')[0];
    const end = (endDate as string) || new Date().toISOString().split('T')[0];
...
router.get('/marketplace/products', authenticateOptional, async (req: AuthRequest, res: Response) => {
  ...
  else if (sortBy === 'rating') list.sort((a, b) => b.rating - a.rating || b.reviewCount - a.reviewCount);
  else if (sortBy === 'latest') list.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
router.get('/marketplace/products/:id', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
...
router.post('/marketplace/products', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
...
router.post('/marketplace/orders', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
...
router.patch('/marketplace/products/:id', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  if (!req.user) {
  ...
  const updated: MarketplaceProduct = {
    ...current,
...
router.post('/marketplace/products/:id/unpublish', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  if (!req.user) {
    return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  }
...
router.post('/marketplace/products/:id/favorite', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  if (!req.user) {
    return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  }
...
router.post('/marketplace/products/:id/reviews', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  const product = marketplaceProducts.find((item) => item.id === req.params.id);
  if (!product) {
    return sendError(res, 'Product not found', 404, 'NOT_FOUND');
...
router.get('/marketplace/orders', async (_req: Request, res: Response) => {
  await loadMarketplace();
  return sendSuccess(res, { orders: marketplaceOrders, total: marketplaceOrders.length });
});

router.get('/marketplace/shop/config', authenticateOptional, async (req: AuthRequest, res: Response) => {
...
router.get('/marketplace/shop/metrics', authenticateOptional, async (_req: AuthRequest, res: Response) => {
  return sendSuccess(res, { metrics: marketplaceShopMetrics, versions: marketplaceShopVersions.slice(-8).reverse() });
});

router.get('/marketplace/shop/themes', authenticateOptional, async (_req: AuthRequest, res: Response) => {
  return sendSuccess(res, { themes: Object.keys(themePresets) });
});

router.put('/marketplace/shop/config', authenticateOptional, async (req: AuthRequest, res: Response) => {
  if (!req.user) {
    return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  }
  ...
  const nextConfig = {
    ...
    theme: (req.body?.theme || marketplaceShopConfig.theme) as MarketplaceShopConfig['theme'],
  } satisfies MarketplaceShopConfig;
...
router.post('/marketplace/shop/config/reset', authenticateOptional, async (req: AuthRequest, res: Response) => {
  if (!req.user) {
    return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  }
  if (marketplaceShopConfig.ownerId && req.user.id !== marketplaceShopConfig.ownerId) {
...
router.post('/marketplace/shop/theme/:theme', authenticateOptional, async (req: AuthRequest, res: Response) => {
  if (!req.user) return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  if (marketplaceShopConfig.ownerId && req.user.id !== marketplaceShopConfig.ownerId) return sendError(res, 'Forbidden', 403, 'FORBIDDEN');
  const theme = req.params.theme as keyof typeof themePresets;
  if (!themePresets[theme]) return sendError(res, 'Theme not found', 404, 'NOT_FOUND');
...
router.get('/marketplace/shop/history', authenticateOptional, async (_req: AuthRequest, res: Response) => {
  return sendSuccess(res, { versions: marketplaceShopVersions.slice().reverse() });
});

router.post('/marketplace/shop/metrics/click', authenticateOptional, async (req: AuthRequest, res: Response) => {
  const kind = String(req.body?.kind || 'featured');
  if (kind === 'announcement') marketplaceShopMetrics.announcementClicks += 1;
  else marketplaceShopMetrics.featuredClicks += 1;
  marketplaceShopMetrics.updatedAt = new Date().toISOString();
  await saveJsonStorage(SHOP_METRICS_STORAGE_KEY, marketplaceShopMetrics);
  return sendSuccess(res, { metrics: marketplaceShopMetrics });
});

router.get('/marketplace/orders/:id', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
...
router.post('/marketplace/orders/:id/fulfill', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  if (!req.user) {
    return sendError(res, 'Authentication required', 401, 'UNAUTHORIZED');
  }
...
router.get('/marketplace/me/listings', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  const currentUserId = req.user?.id;
...
router.get('/marketplace/favorites', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  const currentUserId = req.user?.id;
  if (!currentUserId) {
  ...
  const favoriteIds = new Set(
    (await prisma.marketplaceFavorite.findMany({ where: { user_id: currentUserId } })).map((fav) => fav.product_id)
  );
...
router.get('/marketplace/me/orders', authenticateOptional, async (req: AuthRequest, res: Response) => {
  await loadMarketplace();
  const currentUserId = req.user?.id;
...
router.get('/marketplace/rankings', async (_req: Request, res: Response) => {
  await loadMarketplace();
