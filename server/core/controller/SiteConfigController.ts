...
// Get site config
router.get('/site-config', adminLimiter, authenticate, hasPermission(['admin']), async (req: Request, res: Response) => {
  try {
    ...
    const siteConfig = safeJsonParse(config.value, {});
    res.json({
      success: true,
      data: siteConfig
    });
...
// Save site config
router.post('/site-config', adminLimiter, authenticate, hasPermission(['admin']), csrfProtection, createDuplicateRequestGuard({ ttlSeconds: 30 }), async (req: Request, res: Response) => {
  try {
