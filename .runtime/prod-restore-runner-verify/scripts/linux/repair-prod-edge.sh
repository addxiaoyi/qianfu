...
run_public_verify() {
  ...
  if [[ ! -f "$APP_ROOT/package.json" ]] || { [[ "$has_bundled_diagnose" != "1" ]] && ! command -v npm >/dev/null 2>&1; }; then
    if [[ "$STRICT_PUBLIC_VERIFY" == "1" ]]; then
      log_fail "Cannot run strict public verification: package.json plus bundled node runner or npm is missing"
    fi
    log_warn "Skipping public verification because package.json plus bundled node runner or npm is missing"
    return
  fi
...
run_validation() {
  ...
  if should_repair_web && [[ "$RUN_FRONTEND_PROBE" == "1" && -f "$APP_ROOT/package.json" && "$can_run_frontend_probe" == "1" ]]; then
