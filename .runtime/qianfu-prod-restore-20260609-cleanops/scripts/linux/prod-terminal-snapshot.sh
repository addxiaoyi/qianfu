...
run_optional_local_api() {
  local path="$1"
  printf '+ curl -fsS -H %q -H %q -H %q %q\n' \
    "Host: ${WEB_DOMAIN}" \
    "X-Forwarded-Host: ${WEB_DOMAIN}" \
  ...
  curl -fsS \
    ...
    "http://127.0.0.1:${API_PORT}${path}" || true
}

current_frontend_entry_asset() {
  local manifest="$APP_ROOT/qianfu-liandeng/dist/qianfu-dist-manifest.json"
  if [[ ! -f "$manifest" ]] || ! command -v node >/dev/null 2>&1; then
    return 0
  fi

  node -e "const fs=require('node:fs');const manifest=JSON.parse(fs.readFileSync(process.argv[1],'utf8'));const assets=manifest.entrypointAssets||[];const entry=assets.find((asset)=>/^\\/assets\\/index-[^/]+\\.js$/.test(asset))||assets.find((asset)=>asset.endsWith('.js'))||'';if(entry)process.stdout.write(entry);" "$manifest"
}
...
if command -v curl >/dev/null 2>&1; then
  run_optional_local_api "/api/health"
  run_optional_local_api "/api/ready"
  run_optional_local_api "/health"
else
  echo "curl not found"
fi
...
if command -v curl >/dev/null 2>&1; then
  ...
  frontend_entry_asset="$(current_frontend_entry_asset || true)"
  if [[ -n "$frontend_entry_asset" ]]; then
    public_probe_urls+=("https://${WEB_DOMAIN}${frontend_entry_asset}")
  else
    echo "frontend entry asset unavailable from qianfu-dist-manifest.json"
  fi
  public_probe_urls+=(
    "https://${PAY_DOMAIN}/"
    "https://${PAY_DOMAIN}/api/health"
  )

  for url in "${public_probe_urls[@]}"; do
    printf '%s -> ' "$url"
