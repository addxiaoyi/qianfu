...
import { buildHupijiaoNotifyReplayKey, buildPayProNotifyReplayKey, buildTpayNotifyReplayKey, buildXpayNotifyReplayKey, buildXpayTenantNotifyReplayKey, extractRequestClientIp, isNotifyIpAllowed, resolveNotifyIpAllowlist, } from '../services/paymentCallbackSecurity.js';
...
export const completeExternalPayment = async (req, paymentId, options = {}) => {
    return redisService.withLock(`payment:${paymentId}`, async () => {
        ...
        if (walletAuditEntry) {
            await logDataChange(walletAuditEntry.userId, 'WALLET_DEPOSIT', walletAuditEntry.target, req, walletAuditEntry.before, walletAuditEntry.after);
        }
        if (paymentAuditEntry) {
            await logDataChange(paymentAuditEntry.userId, 'PAYMENT_COMPLETED', paymentAuditEntry.target, req, paymentAuditEntry.before, paymentAuditEntry.after);
        }
        eventService.emitEvent(EVENTS.PAYMENT_SUCCESS, updatedPayment);
