...
export const login = async (req, res, next) => {
    try {
        if (process.env.LOCAL_AUTH_ONLY !== 'true') {
            throw new AppError('Password login is unavailable in current mode', 404, ErrorCode.NOT_FOUND);
        }
        ...
        const token = jwt.sign({ userId: user.id, mode: 'local-auth' }, process.env.JWT_SECRET || 'change-me', { expiresIn: '7d' });
