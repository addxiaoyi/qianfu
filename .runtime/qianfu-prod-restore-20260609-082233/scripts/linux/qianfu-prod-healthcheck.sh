...
check_frontend_freshness() {
  ...
  if [[ ! -f "scripts/probe-frontend-deploy.ts" ]]; then
    fail "Missing scripts/probe-frontend-deploy.ts for frontend freshness probe"
    return
  fi

  if ! command -v npm >/dev/null 2>&1; then
    fail "npm command not found for frontend freshness probe"
    return
  fi
