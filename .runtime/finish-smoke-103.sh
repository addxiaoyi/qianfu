...
TEST_PAYLOAD=$(python3 -c 'import json; print(json.dumps({"planId":"custom","amount":"6.00","paymentMethod":"alipay","provider":"xpay","subject":"Tenant Smoke 103 Final","extra":"tenant-smoke-103-final"}))')
...
XPAY_CALLBACK_STATUS=$(printf '%s' "$XPAY_ORDER" | python3 -c 'import json,sys; data=json.load(sys.stdin); body=data.get("result") or data.get("data") or {}; print(body.get("callbackStatus",""))')
XPAY_ORDER_STATUS=$(printf '%s' "$XPAY_ORDER" | python3 -c 'import json,sys; data=json.load(sys.stdin); body=data.get("result") or data.get("data") or {}; print(body.get("status",""))')
printf 'ORDER_ID=%s\n' "$ORDER_ID"
