...
import { withCache, invalidateCache } from '../services/cache.js';
    ...
    try {
        ...
        if (permissionsRaw) {
            if (Array.isArray(permissionsRaw)) {
                permArray = permissionsRaw.map((p) => String(p).trim()).filter(Boolean);
            }
        ...
        return sendSuccess(res, { ...apiKey, key: plainKey }, 'API key created successfully. Save it now — it won\'t be shown again.');
    }
    catch (error) {
        next(error);
    }
