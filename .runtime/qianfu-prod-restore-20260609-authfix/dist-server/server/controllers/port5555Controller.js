...
import { validatePort5555Access, getPort5555AccessInfo } from '../config/port5555.js';
import { port5555LogQuerySchema, port5555ExportSchema, port5555CleanupSchema, port5555DetailsSchema, port5555StatsQuerySchema } from '../utils/validation.js';
...
export const getPort5555Stats = async (req, res, next) => {
    try {
        ...
        const stats = {
            ...
            topAccessPaths: topAccessPaths.map(item => ({
                path: safeParseDetails(item.details).path,
        ...
        if (req.user) {
            await logAction(req.user.id, 'PORT5555_STATS_VIEWED', 'port5555', req, {
                stats
            });
        }
 ...
 */
export const getPort5555AccessLogs = async (req, res, next) => {
    try {
        if (!req.port5555Access?.hasAccess) {
        ...
        const formattedLogs = logs.map(log => {
            ...
            return {
                ...
                recheckedAt: log.rechecked_at ?? null,
                recheckStatus: log.recheck_status ?? null,
                recheckedBy: log.rechecked_by ?? null
            };
        });
        if (req.user) {
            await logAction(req.user.id, 'PORT5555_LOGS_VIEWED', 'port5555', req, {
                page,
                limit,
                total,
                search,
 ...
 */
export const exportPort5555AccessLogs = async (req, res, next) => {
    try {
        if (!req.port5555Access?.hasAccess) {
            throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
        }
        ...
        const formattedLogs = logs.map(log => {
            ...
            return ({
                ...
                'User Agent': JSON.parse(log.details)?.userAgent || 'Unknown',
        ...
        await logAction(req.user.id, 'PORT5555_LOGS_EXPORTED', 'port5555', req, {
            format,
        ...
        if (format === 'json') {
            ...
            const rows = formattedLogs.map(log => Object.values(log).map(value => {
 ...
 */
export const cleanupPort5555Logs = async (req, res, next) => {
    try {
        if (!req.port5555Access?.hasAccess) {
            throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
        }
        ...
        await logAction(req.user.id, 'PORT5555_LOGS_CLEANED', 'port5555', req, {
            retentionDays,
            cleanupDate,
 ...
 */
export const getPort5555Config = async (req, res, next) => {
    try {
        if (!req.port5555Access?.hasAccess) {
            throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
        }
        import('../config/port5555.js').then(({ PORT_5555_CONFIG }) => {
            logAction(req.user.id, 'PORT5555_CONFIG_VIEWED', 'port5555', req, {});
 ...
 */
export const recheckPort5555AccessLog = async (req, res, next) => {
    try {
        if (!req.port5555Access?.hasAccess) {
            throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
        }
        ...
        await logAction(req.user.id, 'PORT5555_LOG_RECHECKED', 'port5555', req, {
            logId,
            actualGranted,
            expectedGranted,
            mismatch,
...
//# sourceMappingURL=port5555Controller.js.map