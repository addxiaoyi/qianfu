...
const ASSET_EXTENSION_MIME_ALLOWLIST = {
    '.png': ['image/png'],
    '.jpg': ['image/jpeg'],
    '.jpeg': ['image/jpeg'],
    '.gif': ['image/gif'],
...
export class UploadService {
    static uploadsDir = path.resolve(__dirnameResolved, '../../uploads');
    ...
    static checkMagicNumbers(buffer) {
        if (buffer.length < 4)
            return null;
        ...
        if (hex === '47494638')
            return 'image/gif';
        if (hex === '52494646' && buffer.toString('utf8', 8, 12) === 'WEBP')
            return 'image/webp';
        return null;
    }
    static detectAssetMime(buffer, filename) {
    ...
    static isAssetMimeCompatible(ext, mime) {
        const compatibleMimes = ASSET_EXTENSION_MIME_ALLOWLIST[ext];
        return Boolean(compatibleMimes?.includes(mime));
    }
    static async processAndSaveImage(buffer, originalFilename, userId) {
        ...
        if (!moderationResult.passed)
            throw new Error(moderationResult.reason || 'Image content violates safety guidelines');
        ...
        if (processedBuffer.length > MAX_FILE_SIZE)
            throw new Error('Processed file size too large');
        ...
        if (!finalAbsPath.startsWith(this.uploadsDir))
            throw new Error('Invalid file path calculation');
    ...
    static async processAndSaveAsset(buffer, originalFilename) {
        const isSafe = await this.scanForViruses(buffer);
        if (!isSafe)
            throw new Error('File failed virus scan');
        const type = await fileTypeFromBuffer(buffer);
