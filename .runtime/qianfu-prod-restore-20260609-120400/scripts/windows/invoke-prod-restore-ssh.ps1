...
Write-Host "[restore-ssh] target=$target port=$Port bundle=$resolvedBundle remoteBundle=$RemoteBundlePath remoteAppRoot=$RemoteAppRoot dryRun=$DryRun"
...
if (-not $SnapshotOnly) {
    $remoteCommands += "bash scripts/linux/prod-terminal-minimal-repair.sh --preflight-only"
}

if (-not $SnapshotOnly -and -not $PreflightOnly) {
    $remoteCommands += "bash scripts/linux/prod-terminal-minimal-repair.sh --dry-run --no-strict"
}

if (-not $SnapshotOnly -and -not $PreflightOnly -and -not $NoRepair) {
    $repairCommand = "bash scripts/linux/prod-terminal-minimal-repair.sh"
    if ($NoStrict) {
        $repairCommand = "$repairCommand --no-strict"
    }
    if ($User -ne "root") {
