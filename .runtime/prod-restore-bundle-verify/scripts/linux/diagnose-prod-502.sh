...
probe_frontend_deploy() {
  ...
  local output
  if ! output="$(npm run --silent probe:frontend-deploy -- --report-only --kv 2>/tmp/qianfu-diagnose-frontend-probe.err)"; then
...
probe_unified_public_diagnose() {
  ...
  local output
  if ! output="$(QIANFU_BASE_URL="https://${WEB_DOMAIN}" PAY_DOMAIN_HOST="${PAY_DOMAIN}" PAY_MAIN_SITE_HOST="${WEB_DOMAIN}" npm run --silent prod:diagnose:public -- --report-only --kv 2>/tmp/qianfu-diagnose-public.err)"; then
