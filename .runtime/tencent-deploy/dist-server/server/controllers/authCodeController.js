...
import { sendPhoneLoginCode } from '../services/smsService.js';
...
function parseIdentifier(body) {
    ...
    if (email) {
        return { identifier: normalizeEmail(email), type: 'email', address: normalizeEmail(email) };
    }
    if (phone) {
        return { identifier: normalizePhone(phone), type: 'phone', address: normalizePhone(phone) };
    }
 ...
 */
export const sendLoginCode = async (req, res, next) => {
    try {
        ...
        if (!user) {
            return sendSuccess(res, { needsRegistration: true, [type]: address }, 'Account not registered — please register first');
        }
        // 检查发送频率限制
        const now = Date.now();
...
export const verifyLoginCode = async (req, res, next) => {
    try {
        ...
        if (user.verification_token !== generateCodeHash(address, code)) {
            ...
            const updateData = { login_count: nextCount };
        ...
        if (stUserId) {
            try {
                await Session.createNewSession(req, res, process.env.NEXT_PUBLIC_SUPER_TOKENS_TENANT_ID || '1', stUserId);
...
//# sourceMappingURL=authCodeController.js.map