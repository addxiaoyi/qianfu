import { validatePort5555Access, getPort5555AccessInfo, PORT_5555_CONFIG } from '../config/port5555.js';
...
export const port5555Auth = async (req, res, next) => {
    try {
        ...
        const hasAccess = validatePort5555Access(user.role, userPermissions);
        logger.debug(`[Port5555Auth] Has access: ${hasAccess}`);
        if (!hasAccess) {
            // Record access denial
            await logAction(user.id, 'PORT5555_ACCESS_DENIED', 'port5555', req, {
                ipAddress: req.ip,
                userAgent: req.get('User-Agent'),
                path: req.path,
                method: req.method,
                accessInfo: getPort5555AccessInfo(user.role, userPermissions)
            });
        ...
        await logAction(user.id, 'PORT5555_ACCESS_GRANTED', 'port5555', req, {
            ipAddress: req.ip,
            userAgent: req.get('User-Agent'),
            path: req.path,
            method: req.method
...
export const port5555Session = async (req, res, next) => {
    try {
        const isPort5555Request = req.headers['x-port-5555'] === 'true' ||
            req.path.startsWith('/api/port5555');
        if (!isPort5555Request) {
        ...
        if (sessionStart) {
            ...
            if (sessionAge > PORT_5555_CONFIG.ACCESS_CONTROL.SESSION_TIMEOUT) {
                // Record access denial
                await logAction(req.user?.id || 0, 'PORT5555_SESSION_EXPIRED', 'port5555', req, {
                    ipAddress: req.ip,
                    sessionAge
                });
...
export const port5555RateLimit = async (req, res, next) => {
    try {
        const isPort5555Request = req.headers['x-port-5555'] === 'true' ||
            req.path.startsWith('/api/port5555');
        if (!isPort5555Request) {
        ...
        if (count > PORT_5555_CONFIG.ACCESS_CONTROL.RATE_LIMIT.max) {
            ...
            await logAction(req.user?.id || 0, 'PORT5555_RATE_LIMIT_EXCEEDED', 'port5555', req, {
                ipAddress: req.ip,
                requestCount: count,
                limit: PORT_5555_CONFIG.ACCESS_CONTROL.RATE_LIMIT.max
            });
            throw new AppError(PORT_5555_CONFIG.ACCESS_CONTROL.RATE_LIMIT.message, 429, ErrorCode.TOO_MANY_REQUESTS);
        }
...
export const port5555SecurityHeaders = (req, res, next) => {
    const isPort5555Request = req.headers['x-port-5555'] === 'true' ||
        req.path.startsWith('/api/port5555');
    if (!isPort5555Request) {
        return next();
...
//# sourceMappingURL=port5555Auth.js.map