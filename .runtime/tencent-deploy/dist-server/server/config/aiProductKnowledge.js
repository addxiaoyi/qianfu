import { AI_INTEGRATION_REGISTRY, formatAiRegistryForPrompt, formatAiRegistrySubsetForPrompt, } from './aiIntegrationRegistry.js';
/** 安全与回答骨架（与智谱对接共用） */
...
`.trim();
export function buildAiSystemPromptBase(language, activeIntegrationIds) {
    const langLine = language === 'en'
        ? 'Answer in English unless the user writes in Chinese.'
        : '用户使用中文时优先用中文回答。';
    const hasSubset = Array.isArray(activeIntegrationIds) && activeIntegrationIds.length > 0;
...
export function buildFullAiSystemPrompt(language, activeIntegrationIds) {
    return `${AI_CORE_INSTRUCTIONS}

${buildAiSystemPromptBase(language, activeIntegrationIds)}`.trim();
}
//# sourceMappingURL=aiProductKnowledge.js.map