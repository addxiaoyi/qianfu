...
import { getDatabaseUrl, isMySqlUrl, isPostgresUrl, isSqliteUrl } from '../server/utils/dbProvider.js';
...
function parseSqliteFilePath(databaseUrl) {
...
function maskPgUrl(databaseUrl) {
    return databaseUrl.replace(/\/\/([^:@/]+)(?::[^@/]+)?@/, '//***:***@');
}
function backupSqlite(databaseUrl, timestamp) {
...
function backupPostgres(databaseUrl, timestamp) {
...
function backupMySql(databaseUrl, timestamp) {
    ...
    fs.writeFileSync(backupFile, gzip.stdout);
 ...
 */
function rotateBackups(extension) {
    try {
