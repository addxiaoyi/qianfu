...
function createSignedToken(token, secret) {
    const hmac = crypto.createHmac('sha256', secret);
    hmac.update(token);
    return `${token}.${hmac.digest('hex')}`;
}
function verifySignedToken(tokenWithSig, secret) {
    if (!tokenWithSig || !secret)
        return false;
...
function shouldSkipCSRF(req) {
    ...
    const bypassCSRF = (process.env.CSRF_BYPASS_PATHS || '')
        .split(',')
        .map((path) => path.trim())
        .filter(Boolean);
    return process.env.NODE_ENV !== 'production' && bypassCSRF.some((path) => req.path.startsWith(path));
}
export const csrfProtection = async (req, res, next) => {
    ...
    const attackCount = await redisService.get(cacheKey) || 0;
    if (attackCount >= 5) {
    ...
    const tokenFromHeader = req.headers[CSRF_TOKEN_HEADER] || req.headers['X-CSRF-Token'];
...
export const cleanupCsrfCache = async () => {
    try {
...
export const generateCsrfTokens = (req, res, next) => {
...
export const rotateCsrfToken = (req, res, next) => {
    ...
    req.csrfToken = newSignedToken;
    next();
};
export const clearCsrfTokens = (res) => {
    const isSecure = process.env.NODE_ENV === 'production' && process.env.FORCE_HTTPS === 'true';
    res.clearCookie(CSRF_SECRET_COOKIE, {
        httpOnly: true,
        secure: isSecure,
...
export const validateCsrfOrigin = (req, _res, next) => {
    if (req.method === 'GET' || req.method === 'HEAD' || req.method === 'OPTIONS') {
        return next();
    }
