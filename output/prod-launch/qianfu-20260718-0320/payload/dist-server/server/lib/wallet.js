...
import { fenToYuanNumber } from '../utils/currency.js';
...
const HMAC_SECRET = (() => {
    ...
    if (process.env.NODE_ENV === 'production') {
        throw new Error('[Wallet] FATAL: WALLET_SECRET environment variable is required in production mode');
    }
    const fallback = crypto.randomBytes(32).toString('hex');
    console.warn(`[Wallet] WARNING: Using randomly-generated WALLET_SECRET fallback. Set WALLET_SECRET in production!`);
    return fallback;
})();
/**
 ...
 */
export function verifyTransactionIntegrity(transaction) {
 ...
 */
export async function getWalletBalanceBreakdown(userId) {
 ...
 */
export async function deposit(userId, amount, description = 'Recharge', options) {
    if (amount <= 0)
 ...
 * @param amount Amount in yuan (will be converted to fen for storage)
 */
export async function pay(userId, amount, description = 'Payment') {
    if (amount <= 0)
        throw new Error('Amount must be positive');
    // Convert yuan to fen for storage
    const amountFen = Math.round(amount * 100);
