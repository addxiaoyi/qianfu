...
const BLOCKED_CALLBACK_HOSTS = new Set([
    'localhost',
    'metadata.google.internal',
]);
const resolveCallbackAddresses = async (hostname) => {
    const records = await lookup(hostname, { all: true, verbatim: true });
    return records.map(({ address }) => address);
};
function isBlockedCallbackHostname(hostname) {
    const normalized = hostname.trim().toLowerCase().replace(/\.$/, '');
    return (BLOCKED_CALLBACK_HOSTS.has(normalized) ||
...
export function assertSafeOutboundCallbackUrl(url) {
    ...
    if (isBlockedCallbackHostname(u.hostname)) {
...
export async function assertSafeResolvedOutboundCallbackUrl(url, resolveAddresses = resolveCallbackAddresses) {
    ...
    try {
        ...
        throw new Error('Callback URL hostname could not be resolved');
    }
    if (addresses.length === 0) {
        throw new Error('Callback URL hostname did not resolve to an address');
    }
    if (addresses.some((address) => isPrivateIP(address))) {
        throw new Error('Callback URL resolved to a forbidden internal address');
    }
}
