...
export const registerSchema = z.object({
    ...
    phone: z.string().trim().regex(/^\+?\d{7,15}$/, { message: "Invalid phone number" }).optional(),
    code: z.string().trim().regex(/^\d{6}$/, { message: "Code must contain 6 digits" }).optional(),
    password: strongPasswordSchema,
    confirmPassword: z.string().optional(),
    username: z.string().trim().min(3, { message: "Username must be at least 3 characters" }).max(30)
        .regex(/^[a-zA-Z0-9_-]+$/, 'Username can only contain letters, numbers, underscores and hyphens'),
    agree: z.boolean().optional(),
}).strict().refine((body) => !body.confirmPassword || body.confirmPassword === body.password, {
...
export const loginSchema = z.object({
    identifier: z.string().trim().min(1, { message: "Identifier is required" }).max(320).optional(),
...
}).strict().refine((body) => Boolean(body.identifier || body.email), {
...
const authAddressFields = {
    email: z.string().trim().email({ message: "Invalid email address" }).optional(),
    phone: z.string().trim().regex(/^\+?\d{7,15}$/, { message: "Invalid phone number" }).optional(),
};
export const authCodeRequestSchema = z.object(authAddressFields).strict().refine((body) => Number(Boolean(body.email)) + Number(Boolean(body.phone)) === 1, { message: 'Provide exactly one email or phone number' });
export const authCodeVerifySchema = z.object({
    ...authAddressFields,
    code: z.string().trim().regex(/^\d{6}$/, { message: 'Code must contain 6 digits' }),
}).strict().refine((body) => Number(Boolean(body.email)) + Number(Boolean(body.phone)) === 1, { message: 'Provide exactly one email or phone number' });
...
export const resetPasswordCodeSchema = z.object({
    ...
    confirmPassword: z.string().optional(),
}).strict().refine((body) => Boolean(body.password || body.newPassword), {
    message: 'Password is required',
    path: ['password'],
}).refine((body) => !body.confirmPassword || body.confirmPassword === (body.newPassword || body.password), {
    message: 'Passwords do not match',
    path: ['confirmPassword'],
});
export const resetPasswordTokenSchema = z.object({
    token: z.string().trim().min(32, { message: 'Reset token is invalid' }).max(256),
...
export const paymentCreateSchema = z.object({
    ...
    marketplaceOrderId: z.string().min(1, "Marketplace order ID is required").max(120, "Marketplace order ID too long").optional(),
    ...
    if (value.planId === 'marketplace') {
        if (!value.marketplaceOrderId) {
            context.addIssue({ code: 'custom', path: ['marketplaceOrderId'], message: 'Marketplace order ID is required' });
        }
        if (value.amount !== undefined) {
            context.addIssue({ code: 'custom', path: ['amount'], message: 'Marketplace amount is determined by the order' });
        }
        return;
    }
    if (value.amount === undefined) {
...
const parseQueryPage = (v) => Math.max(1, parseInt(v || '1', 10) || 1);
const parseQueryLimit = (v, fallback = 20) => Math.max(1, Math.min(100, parseInt(v || String(fallback), 10) || fallback));
const parseQueryDate = (v) => (v && !Number.isNaN(Date.parse(v)) ? new Date(v) : undefined);
