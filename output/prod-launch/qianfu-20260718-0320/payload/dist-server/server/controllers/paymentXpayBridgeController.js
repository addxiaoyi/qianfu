...
const parseBridgeProjectConfig = (projectKey, raw) => {
    if (!raw?.trim()) {
...
const getBridgeProjectConfig = async (projectKeyRaw) => {
...
const buildSortedSignBase = (params) => Object.keys(params)
    ...
    .join('&');
const generateHexHmac = (payload, secret) => crypto.createHmac('sha256', secret).update(payload).digest('hex');
...
const normalizeSuccessStatus = (raw) => {
...
const extractOrderIdFromBody = (projectKey, body) => {
...
const ensureBridgeConfig = (config) => {
...
const assertFreshTimestamp = (timestamp) => {
...
const verifyBridgeSignature = (payload, sign, secret) => {
    const signPayload = {
...
const reserveBridgeReplay = async (input, sign) => {
    ...
    const accepted = await redisService.setIfNotExists(replayKey, { projectKey: input.projectKey, orderId: input.orderId, tradeNo: input.tradeNo, amount: input.amount, timestamp: input.timestamp }, BRIDGE_REPLAY_TTL_SECONDS);
    return accepted ? replayKey : '';
};
const forwardToXpayGateway = async (input, config) => {
...
export const xpayGatewayBridgeNotify = async (req, res, next) => {
    let replayKey = null;
    try {
...
export const personalQrListenerNotify = async (req, res, next) => {
    let replayKey = null;
    try {
        const body = req.body && typeof req.body === 'object' ? req.body : {};
        const projectKey = sanitizeProjectKey(String(body.projectKey || DEFAULT_PAYMENT_PROJECT_KEY));
...
//# sourceMappingURL=paymentXpayBridgeController.js.map