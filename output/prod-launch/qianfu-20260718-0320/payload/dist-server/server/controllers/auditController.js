...
import { AuditAnalyzer } from '../config/auditConfig.js';
import { AppError, ErrorCode } from '../utils/errors.js';
import { sendSuccess, sendListResponse } from '../utils/response.js';
import { logAction } from '../services/auditService.js';
import { withCache } from '../services/cache.js';
import { auditLogQuerySchema, auditStatsQuerySchema, auditTimeSeriesQuerySchema, auditReportSchema, auditCleanupSchema, auditExportSchema } from '../utils/validation.js';
import { buildDateRange, buildPagination, resolveSortField, resolveSortOrder, } from '../utils/queryBuilder.js';
...
export const getAuditLogs = async (req, res, next) => {
    try {
        ...
        const { action, userId, search, startDate, endDate, level, sortBy, sortOrder, page, limit } = validation.data;
        ...
        const normalizedSortField = resolveSortField(sortBy, ['created_at', 'action'], 'created_at');
...
export const getAuditStats = async (req, res, next) => {
    try {
        ...
        const stats = await withCache(cacheKey, async () => {
            ...
            // 批量查询用户信息，避免 N+1 问题
            const userIds = eventsByUser.map(item => item.user_id).filter(Boolean);
            ...
            // 使用 Map 快速查找用户，提升查询效率
            const userMap = new Map(users.map(user => [user.id, user]));
            const userEvents = eventsByUser.map(item => {
                if (!item.user_id)
                    return null;
 ...
 */
export const getAuditTimeSeries = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 401, ErrorCode.UNAUTHORIZED);
        }
        ...
        const data = await withCache(cacheKey, async () => {
            ...
            if (provider === 'sqlite') {
                ...
                results = await prisma.$queryRawUnsafe(`SELECT strftime(?, created_at) as time, count(*) as count
           FROM AuditLog
           WHERE created_at >= ?
           GROUP BY time
           ORDER BY time ASC`, dateFormat, startDate.toISOString());
            }
            else if (provider === 'mysql') {
                const dateFormat = interval === 'hour' ? '%Y-%m-%d %H:00' : '%Y-%m-%d';
                results = await prisma.$queryRawUnsafe(`SELECT DATE_FORMAT(created_at, ?) as time, COUNT(*) as count
           FROM AuditLog
           WHERE created_at >= ?
           GROUP BY time
           ORDER BY time ASC`, dateFormat, startDate);
            }
            else {
                const dateFormat = interval === 'hour' ? 'YYYY-MM-DD HH24:00' : 'YYYY-MM-DD';
                results = await prisma.$queryRawUnsafe(`SELECT TO_CHAR(created_at, ?) as time, COUNT(*) as count
           FROM "AuditLog"
           WHERE created_at >= ?
           GROUP BY time
           ORDER BY time ASC`, dateFormat, startDate);
...
export const generateAuditReport = async (req, res, next) => {
    try {
        ...
        if (auditEvents.length === MAX_REPORT_RECORDS) {
            report.warning = `Report generated from the latest ${MAX_REPORT_RECORDS} records. Results may be partial.`;
        }
...
export const exportAuditLogs = async (req, res, next) => {
    try {
        ...
        if (format === 'csv') {
            ...
            const csvRows = auditLogs.map(log => `"${log.created_at.toISOString()}","${sanitizeForCsv(log.user?.username || log.user?.email || 'Unknown')}","${sanitizeForCsv(log.action || '')}","${sanitizeForCsv(log.target || '')}","${sanitizeForCsv(log.ip_address || '')}","${sanitizeForCsv(JSON.stringify(log.details) || '')}"`).join('\n');
