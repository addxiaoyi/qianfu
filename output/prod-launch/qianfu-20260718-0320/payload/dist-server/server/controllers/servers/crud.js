...
import { serverSchema, idParamSchema, validateHost, isSafeHostname } from '../../utils/validation.js';
...
import { hookService, MotiaHook } from '../../services/hookService.js';
...
import { getEffectiveServerLimit } from '../../services/userLevelService.js';
import { logger } from '../../utils/logger.js';
import { pay } from '../../lib/wallet.js';
const SERVER_LISTING_PRICE_FEN = {
    'basic-monthly': 700,
    'pro-quarterly': 2000,
    'vip-yearly': 9000,
};
const SERVER_LISTING_DURATION_DAYS = {
    'basic-monthly': 30,
    'pro-quarterly': 90,
    'vip-yearly': 365,
};
...
export const createServer = async (req, res, next) => {
    try {
        ...
        const { name, name_en, thumbnail, summary, summary_en, content_html, ip, group_number, tags, link, activity, platform, category, online_mode, supported_versions, network_env, listing_plan } = validation.data;
        ...
        const listingPlan = (listing_plan || 'basic-monthly');
        ...
        const server = await redisService.withLock(`user:create_server:${user.id}`, async () => {
            ...
            if (!isAdmin) {
                try {
                    await pay(user.id, listingPriceFen / 100, `Server publish plan: ${listingPlan}`);
                }
                catch (error) {
                    throw new AppError('余额不足，无法开通服务器发布套餐', 400, ErrorCode.INSUFFICIENT_FUNDS);
                }
            }
