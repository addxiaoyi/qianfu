...
import { userRoleUpdateSchema, userEmailVerificationUpdateSchema, idParamSchema, userQuerySchema } from '../utils/validation.js';
...
export const updateUserEmailVerification = async (req, res, next) => {
    try {
        const adminUser = req.user;
        if (!adminUser || !adminUser.permissions) {
            throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
        ...
        cacheDelete(USER_STATS_CACHE_KEY);
        await redisService.del(USER_STATS_CACHE_KEY);
        await logDataChange(adminUser.id, 'UPDATE_USER_EMAIL_VERIFIED', `user_${updatedUser.id}`, req, targetUser, updatedUser);
        return sendSuccess(res, {
            user: {
                id: updatedUser.id,
                email: updatedUser.email,
