...
import { sendBatchResponse, sendCreatedResponse, sendDetailResponse, sendListResponse, sendUpdatedResponse, } from '../utils/response.js';
import { getPaginationOptions } from '../utils/pagination.js';
import { z } from 'zod';
// Validation schemas
const createReportSchema = z.object({
    target_type: z.enum(['SERVER', 'USER', 'PRODUCT', 'ORDER', 'MARKETPLACE_REVIEW', 'SELLER']),
    target_id: z.union([
...
export const createReport = async (req, res, next) => {
    try {
        ...
        const { target_type, target_id: rawTargetId, reason, description } = validation.data;
        // Verify target exists
        let targetExists = false;
        let targetId = 0;
        let targetRef = null;
        const numericTargetId = typeof rawTargetId === 'number' ? rawTargetId : Number(rawTargetId);
        const stringTargetId = String(rawTargetId).trim();
        if (target_type === 'SERVER') {
            if (!Number.isSafeInteger(numericTargetId) || numericTargetId <= 0) {
                throw new AppError('Invalid SERVER target ID', 400, ErrorCode.VALIDATION_ERROR);
            }
            const server = await prisma.server.findUnique({ where: { id: numericTargetId } });
            if (server)
                targetExists = true;
            targetId = numericTargetId;
        }
        else if (target_type === 'USER' || target_type === 'SELLER') {
            if (!Number.isSafeInteger(numericTargetId) || numericTargetId <= 0) {
                throw new AppError(`Invalid ${target_type} target ID`, 400, ErrorCode.VALIDATION_ERROR);
            }
            if (numericTargetId === user.id) {
                throw new AppError('You cannot report yourself', 409, ErrorCode.CONFLICT);
            }
            const targetUser = await prisma.user.findUnique({ where: { id: numericTargetId } });
            if (targetUser)
                targetExists = true;
            targetId = numericTargetId;
        }
        else if (target_type === 'PRODUCT') {
            const product = await prisma.marketplaceProduct.findUnique({ where: { id: stringTargetId } });
            if (product)
                targetExists = true;
            targetRef = stringTargetId;
        }
        else if (target_type === 'ORDER') {
            const order = await prisma.marketplaceOrder.findUnique({
                where: { id: stringTargetId },
                include: { product: { select: { creator_id: true } } },
            });
            if (order) {
                const canReportOrder = order.buyer_id === user.id
                    || order.product.creator_id === user.id
                    || req.isAdmin === true;
                if (!canReportOrder) {
        ...
        else if (target_type === 'MARKETPLACE_REVIEW') {
            const review = await prisma.marketplaceReview.findUnique({ where: { id: stringTargetId } });
            if (review)
                targetExists = true;
            targetRef = stringTargetId;
        }
