...
import { rollbackSchema, cmsPaginationQuerySchema, cmsGetPageQuerySchema, saveDraftSchema } from '../utils/validation.js';
...
function hashContent(title, mdText) {
...
export const getPage = async (req, res, next) => {
    try {
        ...
        let page = await redisService.get(cacheKey);
        if (!page) {
            ...
            if (!lock)
                await redisService.set(cacheKey, page, CMS_CACHE_TTL);
        }
        if (lock && page) {
        ...
        if (!page)
            throw new AppError('Page not found', 404, ErrorCode.NOT_FOUND);
...
function buildStaticHtml(title, description, bodyHtml) {
...
export const listAudit = async (req, res, next) => {
    try {
        ...
        if (start || end) {
            filters.created_at = {};
            if (start)
                filters.created_at.gte = new Date(start);
            if (end)
                filters.created_at.lte = new Date(end);
        }
