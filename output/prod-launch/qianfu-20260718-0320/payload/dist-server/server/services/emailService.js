...
import { escapeHtmlChars } from '../utils/xssProtection.js';
...
const EMAIL_TRANSPORT_TIMEOUT_MS = 8_000;
const EMAIL_SEND_TIMEOUT_MS = 7_000;
    ...
    if (runtime.transport.kind === 'smtp') {
        return nodemailer.createTransport({
            ...
            auth: runtime.transport.user
                ...
                : undefined,
            tls: runtime.transport.tlsRejectUnauthorized === false
...
async function sendMailWithinTimeout(transporter, message) {
    let timeoutId;
    ...
    try {
        return await Promise.race([transporter.sendMail(message), timeout]);
    }
    finally {
        if (timeoutId)
...
async function getMailMeta() {
    ...
    return {
        ...
        contactEmail: runtime.meta.contactEmail || '',
        contactPhone: runtime.meta.contactPhone || '',
        emailBaseUrl: runtime.meta.emailBaseUrl || process.env.EMAIL_BASE_URL || `http://localhost:${port}`,
    };
}
async function sendMailSmart(payload, runtimeOverride) {
    ...
    await sendMailWithinTimeout(transporter, {
        from,
        to: payload.to,
        subject: payload.subject,
        html: payload.html,
...
export async function sendDiagnosticEmail(to, subject, message) {
...
export async function sendAdminBroadcastEmail(options) {
    ...
    const safeTitle = mode === 'product'
        ? `产品推广通知 | ${meta.brandName}`
        : mode === 'maintenance'
            ? `系统维护通知 | ${meta.brandName}`
            : options.subject;
    const bodyPrefix = mode === 'product'
...
export const sendEmailLoginCode = async (email, code) => {
    ...
    try {
        ...
        logger.error(`[EmailService] Failed to send login code email to ${masked}: ${msg}`);
        throw error;
    }
};
...
export const sendTicketNotification = async (ticket, user, adminEmails) => {
    ...
    const supportEmail = meta.runtime.transport.kind === 'none'
        ? meta.contactEmail
