...
import { cleanupOldTickets } from '../controllers/ticketController.js';
import { cleanupCsrfCache } from '../middleware/csrf.js';
import { getPaymentExpiredBefore, resolvePaymentTimeoutMinutes } from './paymentTimeoutPolicy.js';
...
async function cleanupKnownUserDependencies(userId) {
    await prisma.$transaction(async (tx) => {
        ...
        const ownedServerIds = ownedServerRows.map((row) => row.id);
        ...
        const ownedTicketIds = ownedTicketRows.map((row) => row.id);
...
async function deleteVictimSafely(victim) {
    try {
...
export async function cleanupExpiredUnverified() {
    const now = new Date();
    ...
    for (const v of victims) {
        try {
            ...
            skippedCount += 1;
            if (isForeignKeyConstraintError(error)) {
                logger.warn(`[CLEANUP ${ts}] skipped unverified user id=${v.id} due to unresolved foreign-key dependencies`);
                continue;
            }
...
export async function cleanupServerStatusHistory() {
    ...
    if (result.count > 0) {
        logger.info(`[CLEANUP ${new Date().toISOString()}] removed ${result.count} server status history rows older than ${SERVER_STATUS_HISTORY_RETENTION_DAYS} days`);
    }
    return result.count;
}
export function startCleanupScheduler(intervalMs = 60000) {
