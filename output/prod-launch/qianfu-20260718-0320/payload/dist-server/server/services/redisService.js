...
class RedisService {
    ...
    reconnectAttempts = 0;
    maxReconnectAttempts = Number.parseInt(process.env.REDIS_MAX_RECONNECT_ATTEMPTS || '5', 10);
            ...
            this.client = new Redis(REDIS_URL, {
                ...
                retryStrategy: (times) => {
                    ...
                    if (times >= this.maxReconnectAttempts) {
                        ...
                        this.logErrorThrottled('[Redis] Connection retry limit reached. Disabling Redis for this process and falling back to memory cache.', { attempts: times, url: REDIS_URL });
                        return null;
                    }
    ...
    logErrorThrottled(message, meta) {
        const now = Date.now();
    ...
    async disconnect() {
        if (this.client) {
            try {
                ...
                logger.warn('[Redis] Error during disconnect:', error.message);
