...
INCLUDE_PACKAGE_FILES="${INCLUDE_PACKAGE_FILES:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
Options:
  ...
  --include-package-files Include package.json/package-lock.json files
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --include-package-files) INCLUDE_PACKAGE_FILES="1"; shift ;;
...
build_restore_runners() {
  local esbuild_bin="$APP_ROOT/node_modules/esbuild/bin/esbuild"
  local runner_dir="$STAGE_ROOT/scripts/prod-restore-runners"

  if [[ "$DRY_RUN" == "1" ]]; then
    printf '[DRY] build restore runners into %s\n' "$runner_dir"
    return
  fi

  [[ -f "$esbuild_bin" ]] || log_fail "Missing esbuild at $esbuild_bin; run npm install before creating the restore bundle"
  mkdir -p "$runner_dir"
  (
    cd "$APP_ROOT"
    node "$esbuild_bin" scripts/probe-frontend-deploy.ts \
      --bundle \
      --platform=node \
      --format=esm \
      --target=node18 \
      --outfile="$runner_dir/probe-frontend-deploy.mjs"
    node "$esbuild_bin" scripts/diagnose-public-prod.ts \
      --bundle \
      --platform=node \
      --format=esm \
      --target=node18 \
      --outfile="$runner_dir/diagnose-public-prod.mjs"
  )
}
...
require_file scripts/remote_restore_password.py
require_file scripts/utils/domain-cert-probe.mjs
require_file scripts/windows/invoke-prod-restore-ssh.ps1
require_file scripts/windows/upload-frontend-dist-ftp.ps1
require_file scripts/windows/verify-public-production.ps1
...
require_file docs/PROD-OPERATOR-CHECKLIST-2026-06-09.md
require_file docs/PROD-RESTORE-UPLOAD-GUIDE-2026-06-09.md
...
copy_path scripts/remote_restore_password.py
copy_path scripts/utils/domain-cert-probe.mjs
copy_path scripts/windows/invoke-prod-restore-ssh.ps1
copy_path scripts/windows/upload-frontend-dist-ftp.ps1
copy_path scripts/windows/verify-public-production.ps1
...
copy_path docs/PROD-OPERATOR-CHECKLIST-2026-06-09.md
copy_path docs/PROD-RESTORE-UPLOAD-GUIDE-2026-06-09.md
build_restore_runners
