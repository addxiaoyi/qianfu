...
import { getLevelProgress, invalidateUserCache, sameCalendarDayInTimeZone, sameUtcCalendarDay, XP_CHECKIN, } from '../services/userLevelService';
...
async function getCurrentStreak(userId) {
    ...
    const rows = await prisma.$queryRawUnsafe('SELECT streak_days FROM checkin_history WHERE user_id = ? ORDER BY checkin_date DESC LIMIT 1', userId);
...
export const getCheckinStatus = async (req, res, next) => {
    try {
        ...
        const [todayRows, latestRows, recentRows] = await Promise.all([
            prisma.$queryRawUnsafe('SELECT id FROM checkin_history WHERE user_id = ? AND checkin_date = ? LIMIT 1', userId, todayKey),
            prisma.$queryRawUnsafe('SELECT streak_days FROM checkin_history WHERE user_id = ? ORDER BY checkin_date DESC LIMIT 1', userId),
            prisma.$queryRawUnsafe('SELECT checkin_date FROM checkin_history WHERE user_id = ? ORDER BY checkin_date DESC LIMIT 30', userId),
        ]);
...
export const postCheckin = async (req, res, next) => {
    try {
        ...
        const hasCheckedInByUser = row.last_checkin_at
            ...
            : false;
        const todayRecordRows = await prisma.$queryRawUnsafe('SELECT * FROM checkin_history WHERE user_id = ? AND checkin_date = ? LIMIT 1', userId, todayKey);
        ...
        const txResult = await prisma.$transaction(async (tx) => {
            const latestRows = await tx.$queryRawUnsafe('SELECT checkin_date, streak_days FROM checkin_history WHERE user_id = ? ORDER BY checkin_date DESC LIMIT 1', userId);
            ...
            await tx.$executeRawUnsafe(`INSERT INTO checkin_history (user_id, checkin_date, timezone, base_reward, bonus_reward, total_reward, streak_days)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, userId, todayKey, clientTz ?? null, baseReward, bonusReward, totalReward, streakDays);
