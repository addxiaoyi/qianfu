...
import { enqueueDnsDeleteTask, listFreeDomainDnsTasks, processFreeDomainTasks } from '../services/freeDomainDnsService.js';
import { getStoredDnsProviderConfig, maskDnsProviderConfig, saveStoredDnsProviderConfig, isCloudflareOauthConfigured, } from '../services/freeDomainDnsProvider.js';
import { hasAuthorizedPermission, isAdministrativeRole } from '../utils/userPermissions.js';
import { redisService } from '../services/redisService.js';
import { revokeCloudflareOauth, saveCloudflareOauthToken } from '../services/freeDomainDnsProvider.js';
...
const oauthStateKey = (state) => `free-domain:cloudflare-oauth:${state}`;
...
function oauthClientId() {
    ...
    if (!value)
        throw new AppError('Cloudflare OAuth 尚未配置 Client ID', 503, ErrorCode.SERVICE_UNAVAILABLE);
    return value;
}
export async function startCloudflareOauth(req, res, next) {
    try {
        const suffixId = idSchema.parse(req.params).id;
        const suffix = await localPrisma.freeDomainSuffix.findUnique({ where: { id: suffixId } });
        if (!suffix || suffix.provider !== 'CLOUDFLARE')
            throw new AppError('该后缀未绑定 Cloudflare', 400, ErrorCode.VALIDATION_ERROR);
...
export async function cloudflareOauthCallback(req, res, next) {
    try {
        const code = typeof req.query.code === 'string' ? req.query.code : '';
        const state = typeof req.query.state === 'string' ? req.query.state : '';
        if (!code || !state)
            throw new AppError('Cloudflare OAuth 回调参数无效', 400, ErrorCode.VALIDATION_ERROR);
        const pending = await redisService.get(oauthStateKey(state));
        if (!pending)
            throw new AppError('Cloudflare OAuth 状态已过期，请重新连接', 400, ErrorCode.VALIDATION_ERROR);
        await redisService.del(oauthStateKey(state));
        const secret = process.env.CLOUDFLARE_OAUTH_CLIENT_SECRET?.trim();
        if (!secret)
            throw new AppError('Cloudflare OAuth 尚未配置 Client Secret', 503, ErrorCode.SERVICE_UNAVAILABLE);
        ...
        if (!response.ok || !payload?.access_token)
            throw new AppError('Cloudflare OAuth 授权失败', 502, ErrorCode.SERVICE_UNAVAILABLE);
...
export async function revokeCloudflareOauthController(req, res, next) {
    try {
        await revokeCloudflareOauth(idSchema.parse(req.params).id);
        return sendSuccess(res, { revoked: true }, 'Cloudflare OAuth 已解除');
    }
    catch (error) {
        return next(error);
    }
}
export async function listFreeDomainSuffixes(_req, res, next) {
    try {
...
export async function listAdminFreeDomainSuffixes(_req, res, next) {
    try {
        const rows = await localPrisma.freeDomainSuffix.findMany({ orderBy: { suffix: 'asc' } });
        const result = await Promise.all(rows.map(async (row) => {
            const providerConfig = await getStoredDnsProviderConfig(row.provider, row.id);
            return {
                ...row,
...
export async function upsertAdminFreeDomainSuffix(req, res, next) {
    try {
        const parsed = suffixSchema.safeParse(req.body);
        if (!parsed.success)
            throw new AppError('免费域名后缀配置无效', 400, ErrorCode.VALIDATION_ERROR, false, parsed.error.issues);
        const { cloudflare_api_token, cloudflare_zone_id, aliyun_access_key_id, aliyun_access_key_secret, aliyun_region_id, ...suffixData } = parsed.data;
        ...
        await saveStoredDnsProviderConfig(row.provider, row.id, {
...
export async function getServerDomainStatus(req, res, next) {
    try {
        if (!req.user)
        ...
        return sendSuccess(res, row ? {
            ...
            lastError: row.tasks.find((task) => task.status === 'FAILED')?.last_error ?? null,
...
export async function retryDnsTask(req, res, next) {
    try {
...
export async function listAdminDnsTasks(_req, res, next) {
    try {
        return sendSuccess(res, await listFreeDomainDnsTasks());
    }
    catch (error) {
        return next(error);
    }
}
export async function revokeServerDomain(req, res, next) {
    try {
        if (!req.user)
            throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
        const serverId = idSchema.parse({ id: req.params.serverId }).id;
        ...
        if (!domain || (domain.user_id !== req.user.id && !isAdmin))
...
export async function runDnsTasks(_req, res, next) {
    try {
        return sendSuccess(res, { processed: await processFreeDomainTasks() }, 'DNS tasks processed');
    }
    catch (error) {
        return next(error);
    }
}
//# sourceMappingURL=freeDomainDnsController.js.map