...
import { sendCreatedResponse, sendDetailResponse, sendSuccess } from '../utils/response';
...
const validateTaskPayload = (body) => { const title = String(body.title ?? '').trim(); const platform = normalizePlatform(String(body.platform ?? '')); const targetId = String(body.targetId ?? '').trim(); const targetUrl = String(body.targetUrl ?? '').trim(); const rewardAmount = Math.trunc(Number(body.rewardAmount)); if (!title || !platform || !targetId || !targetUrl || !Number.isFinite(rewardAmount) || rewardAmount <= 0)
    throw new AppError('Missing or invalid required fields', 400, ErrorCode.VALIDATION_ERROR); return { title, platform, targetId, targetUrl, rewardAmount }; };
export const listPromoTasks = async (req, res, next) => { try {
    const tasks = await prisma.promoTask.findMany({ where: req.isAdmin ? {} : { status: 'ENABLED' }, orderBy: { created_at: 'desc' } });
    const userId = req.user?.id;
    const platformBindings = userId ? await prisma.promoPlatformBinding.findMany({ where: { user_id: userId } }) : [];
    const taskIds = tasks.map((task) => task.id);
    const claims = taskIds.length > 0 ? await prisma.promoClaimRecord.findMany({ where: { task_id: { in: taskIds }, ...(userId ? { user_id: userId } : {}) } }) : [];
    const claimMap = new Map(claims.map((claim) => [claim.task_id, claim]));
    const bindingMap = new Map(platformBindings.map((binding) => [binding.platform, binding]));
    return sendSuccess(res, tasks.map((task) => ({ ...task, rule_config: task.rule_config, claimed: userId ? claimMap.has(task.id) : false, claim_status: claimMap.get(task.id)?.claim_status ?? null, reward_status: claimMap.get(task.id)?.reward_status ?? null, bound: userId ? bindingMap.has(normalizePlatform(task.platform)) : false })), 'Promo tasks loaded');
}
catch (error) {
    next(error);
} };
...
export const bindPlatformAccount = async (req, res, next) => { try {
    if (!req.user)
    ...
    const binding = await prisma.promoPlatformBinding.upsert({ where: { platform_platform_user_id: { platform: normalizedPlatform, platform_user_id: String(platformUserId) } }, update: { user_id: userId, platform_username: platformUsername || null, binding_status: 'ACTIVE', bind_source: 'MANUAL', verified_at: new Date(), last_verify_at: new Date() }, create: { user_id: userId, platform: normalizedPlatform, platform_user_id: String(platformUserId), platform_username: platformUsername || null, binding_status: 'ACTIVE', bind_source: 'MANUAL', verified_at: new Date(), last_verify_at: new Date() } });
...
export const submitPromoClaim = async (req, res, next) => { try {
    if (!req.user)
        throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
    const userId = req.user.id;
    const { taskId, proofData } = req.body ?? {};
    if (!taskId)
        throw new AppError('Missing taskId', 400, ErrorCode.VALIDATION_ERROR);
    ...
    if (task.start_at && task.start_at > new Date())
        throw new AppError('Task has not started yet', 400, ErrorCode.VALIDATION_ERROR);
    if (task.end_at && task.end_at < new Date())
    ...
    const existingClaim = await prisma.promoClaimRecord.findFirst({ where: { user_id: userId, task_id: task.id }, orderBy: { created_at: 'desc' } });
    if (existingClaim)
        return sendSuccess(res, existingClaim, 'Task already claimed');
    const ruleConfig = parseRuleConfig(task.rule_config);
    const verifyResult = task.auto_verify ? verifyActionsLocally(ruleConfig) : { passed: !task.need_audit, actions: {}, condition: 'manual', reason: task.need_audit ? 'Awaiting audit' : '' };
    const claimRequestNo = makeRequestNo();
    const claim = await prisma.$transaction(async (tx) => { const createdClaim = await tx.promoClaimRecord.create({ data: { task_id: task.id, user_id: userId, platform_user_id: binding.platform_user_id, claim_status: verifyResult.passed && !task.need_audit ? 'VERIFIED' : task.need_audit ? 'PENDING' : 'REJECTED', reward_status: verifyResult.passed && !task.need_audit ? 'REWARDING' : 'PENDING', verify_result: JSON.stringify(verifyResult), verify_detail: JSON.stringify({ proofData: proofData ?? null, ruleConfig }), proof_data: proofData ? JSON.stringify(proofData) : null, claim_request_no: claimRequestNo, claim_at: new Date(), verified_at: verifyResult.passed && !task.need_audit ? new Date() : null, rewarding_at: verifyResult.passed && !task.need_audit ? new Date() : null, failed_reason: verifyResult.passed ? null : verifyResult.reason } }); await tx.promoVerifyLog.create({ data: { claim_id: createdClaim.id, task_id: task.id, user_id: userId, platform_user_id: binding.platform_user_id, verify_status: verifyResult.passed ? 'PASSED' : 'FAILED', request_data: JSON.stringify({ proofData: proofData ?? null }), response_data: JSON.stringify(verifyResult), error_message: verifyResult.passed ? null : verifyResult.reason, source: task.auto_verify ? 'AUTO' : 'MANUAL' } }); if (!verifyResult.passed || task.need_audit)
        return createdClaim; const wallet = await tx.wallet.upsert({ where: { user_id: userId }, update: {}, create: { user_id: userId, balance: 0, currency: 'CNY', is_active: true } }); const beforeBalance = wallet.balance; const afterBalance = beforeBalance + task.reward_amount; await tx.wallet.update({ where: { user_id: userId }, data: { balance: { increment: task.reward_amount } } }); await tx.promoWalletTransaction.create({ data: { user_id: userId, change_amount: task.reward_amount, direction: 'INCOME', change_type: 'PROMO_REWARD', ref_type: 'promo_claim_record', ref_id: createdClaim.id, before_balance: beforeBalance, after_balance: afterBalance, remark: `Promo reward for task ${task.id}`, created_by: req.user.id } }); await tx.promoClaimRecord.update({ where: { id: createdClaim.id }, data: { claim_status: 'REWARDED', reward_status: 'REWARDED', rewarded_at: new Date() } }); return createdClaim; });
    return sendCreatedResponse(res, claim, { resource: 'PromoClaimRecord' });
}
catch (error) {
    next(error);
} };
...
export const approvePromoClaim = async (req, res, next) => { try {
    if (!isAdmin(req))
        throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
    const claimId = Number(req.params.id);
    if (!Number.isInteger(claimId))
    ...
    if (!claim)
        throw new AppError('Claim not found', 404, ErrorCode.NOT_FOUND);
    if (claim.reward_status === 'REWARDED')
        return sendSuccess(res, claim, 'Claim already rewarded');
    const updated = await prisma.$transaction(async (tx) => { const wallet = await tx.wallet.upsert({ where: { user_id: claim.user_id }, update: {}, create: { user_id: claim.user_id, balance: 0, currency: 'CNY', is_active: true } }); const beforeBalance = wallet.balance; const afterBalance = beforeBalance + claim.task.reward_amount; await tx.wallet.update({ where: { user_id: claim.user_id }, data: { balance: { increment: claim.task.reward_amount } } }); await tx.promoWalletTransaction.create({ data: { user_id: claim.user_id, change_amount: claim.task.reward_amount, direction: 'INCOME', change_type: 'PROMO_REWARD', ref_type: 'promo_claim_record', ref_id: claim.id, before_balance: beforeBalance, after_balance: afterBalance, remark: `Manual approve for task ${claim.task_id}`, created_by: req.user?.id ?? null } }); return tx.promoClaimRecord.update({ where: { id: claim.id }, data: { claim_status: 'REWARDED', reward_status: 'REWARDED', verified_at: claim.verified_at ?? new Date(), rewarding_at: new Date(), rewarded_at: new Date(), audit_by: req.user?.id ?? null, audit_note: req.body?.remark || 'Approved', failed_reason: null } }); });
    return sendSuccess(res, updated, 'Claim approved and rewarded');
}
catch (error) {
    next(error);
} };
export const rejectPromoClaim = async (req, res, next) => { try {
    if (!isAdmin(req))
        throw new AppError('Forbidden', 403, ErrorCode.FORBIDDEN);
    const claimId = Number(req.params.id);
    if (!Number.isInteger(claimId))
        throw new AppError('Invalid claim ID', 400, ErrorCode.VALIDATION_ERROR);
    const updated = await prisma.promoClaimRecord.update({ where: { id: claimId }, data: { claim_status: 'REJECTED', reward_status: 'PENDING', audit_by: req.user?.id ?? null, audit_note: req.body?.remark || 'Rejected', failed_reason: req.body?.remark || 'Rejected by admin' } });
    return sendSuccess(res, updated, 'Claim rejected');
}
catch (error) {
    next(error);
} };
