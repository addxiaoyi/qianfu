...
import { evaluatePromoRewardTier, normalizePromoMetrics, parsePromoRewardPolicy, } from './promoRewardPolicyService.js';
const MAX_ATTEMPTS = 3;
class SettlementRaceError extends Error {
}
const maxMetrics = (input, previous) => (normalizePromoMetrics({
...
const settleInTransaction = async (tx, claimId, payload, actorId) => {
    ...
    if (claimGuard.count !== 1)
        throw new SettlementRaceError('Concurrent settlement detected');
...
export const recordPromoMetricsAndSettle = async (db, claimId, payload, actorId) => {
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
        try {
            return await db.$transaction((tx) => settleInTransaction(tx, claimId, payload, actorId), { isolationLevel: 'Serializable' });
        }
        catch (error) {
...
//# sourceMappingURL=promoMetricSettlementService.js.map