...
import { buildDnsRecordInputs, isDnsTarget, normalizeFreeDomainRequest, } from './freeDomainDnsPolicy.js';
import { executeDnsTask } from './freeDomainDnsTask.js';
import { getDnsProvider } from './freeDomainDnsProvider.js';
...
export async function createServerDomainApplication(tx, input, values) {
    ...
    if (!suffix)
        throw new AppError('免费域名后缀不存在', 400, ErrorCode.VALIDATION_ERROR);
...
export async function enqueueDnsApplyTask(tx, serverId) {
...
export async function updateServerDomainApplication(tx, input, values) {
...
export async function enqueueDnsDeleteTask(serverId) {
...
function repositoryFor(task) {
    return {
        async saveRecord(taskId, input, providerRecordId) {
            const row = await localPrisma.dnsTask.findUnique({ where: { id: taskId } });
            if (!row)
                throw new Error('DNS task disappeared');
        ...
        async markCompleted(taskId) {
            ...
            if (task.action === 'DELETE') {
                ...
                if (applyTask?.status === 'PENDING' || applyTask?.status === 'PROCESSING')
                    return;
            }
        ...
        async markFailed(taskId, message, retryAt) {
...
export async function processFreeDomainTasks(limit = 10) {
    ...
    for (const task of tasks) {
        ...
        if (!locked.count)
            continue;
        try {
            const provider = await getDnsProvider(task.server_domain.suffix.provider, task.server_domain.suffix.suffix, task.server_domain.suffix_id);
...
export function desiredDnsRecords(domain) {
    return buildDnsRecordInputs(domain);
}
//# sourceMappingURL=freeDomainDnsService.js.map