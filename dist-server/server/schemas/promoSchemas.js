...
const optionalPlainText = z.preprocess((value) => (typeof value === 'string' && value.trim() === '' ? undefined : value), plainText.optional());
const platformUserIdPatterns = {
    bilibili: /^\d{1,20}$/,
...
export const promoMetricSnapshotSchema = z.object({
    ...
    rawSummary: z.string().trim().min(1).max(4_000).refine((value) => !/[<>]/.test(value), 'HTML is not allowed').optional(),
}).strict();
