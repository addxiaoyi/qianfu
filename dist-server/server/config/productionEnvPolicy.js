...
export function validateProductionRuntimeEnv(env) {
    ...
    if (personalFilingMode) {
        ...
        if (providerRaw)
            addError('DEFAULT_PAYMENT_UPSTREAM_PROVIDER', 'Payment providers are disabled in personal filing mode');
        if (backupRaw)
            addError('DEFAULT_PAYMENT_BACKUP_PROVIDER', 'Payment backup providers are disabled in personal filing mode');
        if (redirectHosts.length > 0)
            addError('VITE_ALLOWED_PAYMENT_REDIRECT_HOSTS', 'Payment redirect hosts are disabled in personal filing mode');
        if (payproEnabled)
            addError('PAYPRO_ENABLED', 'PAYPRO_ENABLED must be false in personal filing mode');
        if (clean(env, 'PAY_DOMAIN_HOST'))
            addError('PAY_DOMAIN_HOST', 'Dedicated payment domains are disabled in personal filing mode');
    }
    else {
