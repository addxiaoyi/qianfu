...
async function inspectLevel(source, depth, scope, state) {
    ...
    const zipFile = await (Buffer.isBuffer(source)
        ...
        : openArchivePromise(source.filePath, options)).catch((error) => {
...
function openArchivePromise(filePath, options) {
...
export async function inspectArchiveFile(filePath) {
    const state = {
        entryCount: 0,
        totalExpandedBytes: 0,
        nestedArchiveCount: 0,
