 ...
 */
export function captchaGuard(req, res, next) {
    ...
    verifyCaptchaToken(token, secretKey, req.ip || '')
        .then((isValid) => {
        if (isValid) {
            next();
        }
            ...
            res.status(403).json(buildErrorEnvelope({
...
async function verifyCaptchaToken(_token, _secret, _ip) {
...
//# sourceMappingURL=captchaGuard.js.map