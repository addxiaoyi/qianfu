...
function getRequesterScope(req) {
    const userId = req.user?.id;
    if (userId)
        return `u:${userId}`;
    return `ip:${req.ip || req.socket.remoteAddress || 'unknown'}`;
}
function buildRequestDigest(req, includeBody) {
...
export function createDuplicateRequestGuard(options = {}) {
    ...
    return async (req, res, next) => {
        ...
        try {
            ...
            return res.status(409).json(buildErrorEnvelope({
...
export function createIdempotencyMiddleware(options = {}) {
    ...
    return async (req, res, next) => {
        ...
        if (!idemKey) {
            if (!requireHeader)
                return next();
            return res.status(400).json(buildErrorEnvelope({
        ...
        try {
            const cached = await redisService.get(responseKey);
            if (cached) {
                ...
                void (async () => {
                    try {
                        if (typeof capturedBody !== 'undefined' &&
                            res.statusCode >= 200 &&
                            res.statusCode < 400) {
                            await redisService.set(responseKey, { statusCode: res.statusCode, body: capturedBody }, ttlSeconds);
                        }
                    }
                    finally {
                        await releaseLock();
