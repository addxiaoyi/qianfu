  ...
  return (
    <AnimatePresence mode="wait">
      {isLoading ? (
          ...
          {loadingType === 'spinner' ? (
            <div className="flex flex-col items-center gap-5 sm:gap-6 text-center">
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-black font-mono text-zinc-500 uppercase tracking-[0.35em] animate-pulse italic">正在加载数据</p>
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.28em] italic leading-none">正在准备最新内容，请稍候。</p>
              </div>
            </div>
          ) : (
            <div className="w-full max-w-6xl space-y-8 sm:space-y-12">
              <div className="space-y-4">
                <SkeletonBlock className="h-16 w-1/4 rounded-sm" />
                <SkeletonBlock className="h-4 w-2/3 rounded-sm" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {Array.from({ length: 4 }, (_, index) => (
                  <SkeletonBlock key={index} className="h-48 rounded-[2.5rem]" />
                ))}
              </div>
              <SkeletonBlock className="h-96 rounded-[3.5rem]" />
            </div>
          )}
        </motion.div>
      ) : isLocked ? (
        ...
        <motion.div key="error" role="alert" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center min-h-[420px] gap-6 sm:gap-8 text-center px-6">
          ...
          <div className="max-w-md space-y-3 sm:space-y-4">
            <h3 className="text-3xl sm:text-4xl font-black tracking-tighter uppercase italic leading-none text-zinc-900">连接失败</h3>
            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.28em] italic leading-none">服务器未能返回有效数据。</p>
          ...
          {onRetry ? (
            <button type="button" onClick={onRetry} className="w-full sm:w-auto px-6 sm:px-10 py-4 rounded-2xl bg-black text-white text-[11px] font-semibold uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all shadow-lg shadow-black/10 group">
              <RefreshCcw className="w-4 h-4 group-hover:rotate-180 transition-transform duration-700" /> 重新加载
            </button>
          ) : null}
        </motion.div>
      ) : isEmpty ? (
        <motion.div key="empty" role="status" aria-live="polite" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center min-h-[360px] gap-6 sm:gap-8 text-center px-6">
          ...
          <div className="space-y-2 sm:space-y-3 max-w-md">
            <h3 className="text-2xl sm:text-3xl font-black tracking-tighter italic leading-none text-zinc-900">{emptyTitle}</h3>
            <p className="text-sm font-medium text-zinc-500 leading-6">{emptyDescription}</p>
          </div>
          {emptyAction}
        </motion.div>
      ) : (
