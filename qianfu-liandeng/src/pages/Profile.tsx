           ...
           <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
              <div className="flex items-center gap-4 sm:gap-5">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    ...
                    <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-widest">ID #{profile?.id?.slice(0, 8)}</span>
                  </div>
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-zinc-900">{escapeHtml(profile?.username)}</h1>
                  <div className="flex flex-wrap items-center gap-3 text-sm text-zinc-500 font-medium">
                     ...
                     <span className="truncate">{escapeHtml(profile?.email)}</span>
                  </div>
                </div>
              </div>
              <div className="lg:ml-auto flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                 <button 
                  ...
                  className="w-full sm:w-auto px-5 sm:px-6 py-3.5 rounded-2xl bg-black text-white font-semibold text-sm hover:bg-zinc-800 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                >
                  <GeometricLantern variant="activity" className="w-5 h-5" /> {t('profile.checkin')}
                </button>
                <Link to="/me/edit" className="w-full sm:w-auto px-5 sm:px-6 py-3.5 rounded-2xl border border-zinc-200 bg-white text-zinc-700 font-semibold text-sm hover:border-zinc-400 hover:bg-zinc-50 transition-all flex items-center justify-center gap-3">
                  <GeometricLantern variant="settings" className="w-5 h-5 text-zinc-500" /> 编辑资料
                </Link>
              </div>
           </div>

           <div className="mt-6 sm:mt-8 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 sm:gap-6 items-start">
             <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
               {[
                 { label: '账户状态', value: profile?.email ? '已绑定邮箱' : '未绑定邮箱' },
                 { label: '等级进度', value: `Lv.${profile?.level || 1}` },
                 { label: '签到状态', value: checkinMutation.isPending ? '处理中' : '可签到' },
               ].map((item) => (
                 <div key={item.label} className="rounded-2xl border border-zinc-100 bg-white p-4 sm:p-5">
                   <div className="text-[10px] font-black uppercase tracking-[0.24em] text-zinc-400">{item.label}</div>
                   <div className="mt-2 text-sm sm:text-base font-semibold text-zinc-900">{item.value}</div>
                 </div>
               ))}
             </div>
          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="rounded-[2rem] sm:rounded-[3rem] border border-zinc-100 bg-white p-5 sm:p-6 lg:p-8 shadow-[0_12px_40px_rgba(0,0,0,0.04)]">
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                <button className="py-3.5 rounded-2xl bg-black text-white font-semibold text-sm hover:bg-zinc-800 transition-all active:scale-[0.98]">
                  {t('profile.wallet.fund')}
                </button>
                <Link to="/billing" className="py-3.5 rounded-2xl border border-zinc-200 bg-white text-zinc-700 font-semibold text-sm hover:border-zinc-400 hover:bg-zinc-50 transition-all flex items-center justify-center">
                  账单明细
                </Link>
              </div>
            </div>
          ...
          <div className="space-y-6 sm:space-y-8">
             <div className="rounded-[2rem] sm:rounded-[3rem] border border-zinc-100 bg-white p-5 sm:p-6 lg:p-8 shadow-[0_12px_40px_rgba(0,0,0,0.04)] space-y-6">
                <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-zinc-100">
                   ...
                   <div className="px-3 py-1 rounded-full bg-zinc-100 text-[9px] font-black uppercase tracking-widest text-zinc-400">3 Packets Captured</div>
                </header>
                <div className="space-y-4 sm:space-y-5">
                   {[1, 2, 3].map(i => (
                     <div key={i} className="flex gap-4 items-start rounded-[1.5rem] border border-zinc-100 p-4 sm:p-5 bg-zinc-50/60 group">
                        ...
                        <div className="space-y-2 flex-grow min-w-0">
                           <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                              <p className="text-sm sm:text-base font-semibold text-zinc-900">Handshake_Complete</p>
                              <span className="text-[10px] font-black font-mono text-zinc-400 italic">{20 - i}h ago</span>
                           </div>
                           <p className="text-sm text-zinc-500 leading-6">System successfully acknowledged daily presence signal. Allocation: <span className="text-accent font-semibold">+10 XP / +5 CRED</span></p>
                        </div>
                     </div>
                   ))}
                </div>
             ...
             <div className="rounded-[2rem] sm:rounded-[3rem] border border-zinc-100 bg-white p-5 sm:p-6 lg:p-8 shadow-[0_12px_40px_rgba(0,0,0,0.04)] space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    { label: '上次签到', value: '今日已签到' },
                    { label: '账户等级', value: `Lv.${profile?.level || 1}` },
                    { label: '最近活跃', value: '1 小时内' },
                  ].map((item) => (
                    <div key={item.label} className="rounded-[1.5rem] border border-zinc-100 bg-zinc-50 p-4">
                      <div className="text-[10px] font-black uppercase tracking-[0.24em] text-zinc-400">{item.label}</div>
