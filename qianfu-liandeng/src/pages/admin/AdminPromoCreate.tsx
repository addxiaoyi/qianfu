  ...
  return (
    <div className="space-y-16 pb-32 bg-white selection:bg-accent selection:text-white">
      <StatusWrapper isLoading={isEditing && !existingTask} isError={false} onRetry={() => undefined}>
        <AdminPageHeader badge="推广任务 / 创建编辑" title={title} description="快速创建一个推广激励任务。你可以先使用默认的三连规则，再按需调整奖励、有效期与审核方式。" statusLabel="任务编辑器已就绪" rightSlot={(
          <div className="flex gap-4">
            {isEditing && <button type="button" onClick={() => resetDraftMutation.mutate()} className="group px-12 py-8 rounded-[3rem] text-[12px] font-black uppercase tracking-[0.5em] transition-all flex items-center gap-6 bg-white border border-zinc-100 hover:border-amber-300 italic active:scale-[0.98]"><GeometricLantern variant="settings" className="w-6 h-6 group-hover:rotate-12 transition-transform duration-500" />重置草稿</button>}
            {!isEditing && <button type="button" onClick={() => saveDraftMutation.mutate()} className="group px-12 py-8 rounded-[3rem] text-[12px] font-black uppercase tracking-[0.5em] transition-all flex items-center gap-6 bg-white border border-zinc-100 hover:border-accent italic active:scale-[0.98]"><GeometricLantern variant="settings" className="w-6 h-6 group-hover:rotate-12 transition-transform duration-500" />保存草稿</button>}
            <button type="button" onClick={() => submitMutation.mutate()} className="group px-12 py-8 btn-accent rounded-[3rem] text-[12px] font-black uppercase tracking-[0.5em] transition-all flex items-center gap-6 shadow-2xl shadow-accent/20 italic active:scale-[0.98]"><GeometricLantern variant="spark" className="w-6 h-6 group-hover:rotate-12 transition-transform duration-500" />{isEditing ? '更新任务' : '发布任务'}</button>
          </div>
        )} />
