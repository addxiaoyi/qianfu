...
import { getServerName, getServerPlayersOnline, getServerSummary, getServerThumbnail } from '@/utils/serverView';
...
const formatDate = (value: unknown) => {
  ...
  return Number.isNaN(date.getTime()) ? '加入时间未知' : date.toLocaleDateString('zh-CN');
};
      ...
      <div className="max-w-7xl mx-auto px-6 py-32 bg-white selection:bg-accent selection:text-white">
        ...
        <div className="bg-black rounded-[4rem] p-16 text-white mb-24 relative overflow-hidden shadow-2xl shadow-black/20 group">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
              <div className="w-40 h-40 bg-zinc-900 rounded-[2.5rem] border-4 border-zinc-800 flex items-center justify-center text-6xl font-black italic shadow-2xl rotate-3 group-hover:rotate-0 transition-transform duration-700 overflow-hidden">
                 {isImageUrlSafe(profile?.avatar_url) ? <img src={profile.avatar_url} alt={`${profile.username || '用户'} 头像`} className="w-full h-full object-cover" /> : profile?.username?.[0]}
              </div>

              <div className="flex-grow space-y-6">
                 <div className="flex flex-col md:flex-row md:items-center gap-6">
                    <h1 className="text-6xl font-black tracking-tighter uppercase italic leading-none">{profile?.username}</h1>
                    <div className="px-5 py-2 bg-accent/20 border border-accent/30 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] text-accent italic">Lv.{profile?.level ?? 1} Member_Node</div>
                 </div>
                 <p className="text-xl text-zinc-400 font-bold italic leading-relaxed max-w-2xl">{stripHtml(profile?.bio) || t('profile.public.mysterious')}</p>
                 <div className="flex flex-wrap justify-center md:justify-start gap-8 pt-4">
                    <div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.3em] text-zinc-500 italic">
                       <GeometricLantern variant="activity" className="w-5 h-5 text-accent" /> {t('profile.public.joined')} {formatDate(profile?.joinDate)}
                    </div>
                    <div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.3em] text-zinc-500 italic">
                       <GeometricLantern variant="spark" className="w-5 h-5 text-accent" /> {t('profile.public.likes')} {profile?.totalLikes ?? 0}
                    </div>
                 </div>
              </div>
           </div>
        ...
        <div className="space-y-16">
          <h2 className="text-5xl font-black italic uppercase tracking-tighter flex items-center gap-6">
           ...
           {profile?.servers?.length ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {profile?.servers?.map((server: any, i: number) => (
              ...
              >
                 <div className="aspect-video bg-zinc-50 relative overflow-hidden">
                    {getServerThumbnail(server) ? (
                       <img src={getServerThumbnail(server)} alt={`${getServerName(server)} 展示图`} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-1000" />
                    ) : (
                 ...
                 <div className="p-10 space-y-8">
                     <h3 className="text-2xl font-black italic uppercase tracking-tight leading-none">{getServerName(server)}</h3>
                     <p className="line-clamp-2 text-sm leading-6 text-zinc-500">{getServerSummary(server)}</p>
                     <div className="flex justify-between items-center text-[10px] font-black text-zinc-400 uppercase tracking-widest italic">
                       <span className="flex items-center gap-2"><GeometricLantern variant="user" className="w-4 h-4 text-accent" /> {getServerPlayersOnline(server)}</span>
                       <span className="flex items-center gap-2"><GeometricLantern variant="spark" className="w-4 h-4 text-accent" /> {server.like_count ?? 0}</span>
                     </div>
          ...
          </div> : <div className="border border-dashed border-zinc-200 rounded-3xl p-12 text-center text-zinc-400 font-bold">该用户暂未发布已审核服务器。</div>}
        </div>
      </div>
    </StatusWrapper>
  );
