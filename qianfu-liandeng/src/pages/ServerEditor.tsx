...
const editorSections = [
  { id: 'cover', icon: Layout, label: 'Hero_Matrix' },
  { id: 'basic', icon: Settings, label: 'Core_Telemetry' },
  { id: 'content', icon: FileText, label: 'Stream_Content' },
] as const;
...
const ServerEditor: React.FC = () => {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      if (serverId) {
        // EDIT MODE: Only send editable fields — ip/version are verified data, not to be tampered with
        ...
        await api.put(`/servers/${serverId}`, editablePayload);
      } else {
      ...
      toast({ 
        ...
        description: serverId ? 'NODE_IDENTITY_UPDATED' : 'Identity Matrix Broadcast successful.'
      });
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-8 pt-20 pb-16 selection:bg-accent selection:text-white">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-20">
          ...
          <section id="basic" ref={sectionRefs.basic} className="space-y-12 scroll-mt-24">
             <h2 className="text-xs font-black font-mono uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-4 italic">
                <GeometricLantern variant="activity" className="w-5 h-5 text-accent" /> 核心数据 / Core_Telemetry
             </h2>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-16">
                  ...
                  <div className="relative group/field">
                    ...
                    {serverId && (
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover/field:opacity-100 transition-opacity">
                         <span className="text-[8px] font-black uppercase tracking-widest text-zinc-400 bg-white px-2 py-1 border border-zinc-100 rounded-sm">VERIFIED_DATA_LOCKED</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative group/field">
                    ...
                    {serverId && (
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover/field:opacity-100 transition-opacity">
                         <span className="text-[8px] font-black uppercase tracking-widest text-zinc-400 bg-white px-2 py-1 border border-zinc-100 rounded-sm">TELEMETRY_LOCKED</span>
                      </div>
                    )}
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-3">
                      {connectionStatus === 'online' && (
                        <div className="flex items-center gap-2 px-3 py-1 bg-green-50 text-green-600 rounded-sm animate-in fade-in duration-500">
                          ...
                          <span className="text-[8px] font-black uppercase italic">Online</span>
                        </div>
                      )}
                      {connectionStatus === 'offline' && (
                        <div className="flex items-center gap-2 px-3 py-1 bg-red-50 text-red-600 rounded-sm animate-in fade-in duration-500">
                          ...
                          <span className="text-[8px] font-black uppercase italic">Offline</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {/* Form Actions */}
          <div className="flex flex-col sm:flex-row gap-8 pt-12 pb-32">
            <button 
              type="submit"
              disabled={loading}
