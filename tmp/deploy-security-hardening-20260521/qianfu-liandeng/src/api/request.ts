...
export function getLocalAuthToken() {
  if (typeof window === 'undefined') return null;
  const sessionToken = getSessionStorage()?.getItem(LOCAL_AUTH_TOKEN_KEY);
  if (sessionToken) return sessionToken;

  const legacyLocalToken = getLocalStorage()?.getItem(LOCAL_AUTH_TOKEN_KEY);
  if (legacyLocalToken) {
    getSessionStorage()?.setItem(LOCAL_AUTH_TOKEN_KEY, legacyLocalToken);
...
export function setLocalAuthToken(token: string | null) {
  ...
  getSessionStorage()?.setItem(LOCAL_AUTH_TOKEN_KEY, token);
}
