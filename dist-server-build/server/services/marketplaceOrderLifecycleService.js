...
const isRetryableTransactionError = (error) => (typeof error === 'object'
    && error !== null
    && 'code' in error
    && error.code === 'P2034');
const runSerializableTransaction = async (db, operation) => {
    let lastError;
...
const auditData = (context, action, target, details) => ({
...
export const fulfillMarketplaceOrder = async (db, input) => {
...
export const openMarketplaceDispute = async (db, input) => {
    const initial = await db.marketplaceOrder.findUnique({
        where: { id: input.orderId },
        include: { product: true },
    });
...
export const resolveMarketplaceDispute = async (db, input) => {
    const initial = await db.marketplaceOrder.findUnique({
        where: { id: input.orderId },
        include: { product: true },
    });
    ...
    return runSerializableTransaction(db, async (tx) => {
        ...
        if (initial.buyer_id)
            recipientIds.add(initial.buyer_id);
        if (initial.product.creator_id)
            recipientIds.add(initial.product.creator_id);
...
//# sourceMappingURL=marketplaceOrderLifecycleService.js.map