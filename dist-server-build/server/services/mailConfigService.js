...
const MAIL_FIELD_SPECS = [
    ...
    { field: 'fromName', key: `${MAIL_CONFIG_PREFIX}from_name`, type: 'string', description: 'Default sender display name' },
    { field: 'emailFrom', key: `${MAIL_CONFIG_PREFIX}email_from`, type: 'string', description: 'Default from address' },
    { field: 'replyTo', key: `${MAIL_CONFIG_PREFIX}reply_to`, type: 'string', description: 'Reply-to address' },
    { field: 'imapHost', key: `${MAIL_CONFIG_PREFIX}imap_host`, type: 'string', description: 'IMAP host' },
    { field: 'imapPort', key: `${MAIL_CONFIG_PREFIX}imap_port`, type: 'number', description: 'IMAP port' },
    { field: 'imapSecure', key: `${MAIL_CONFIG_PREFIX}imap_secure`, type: 'boolean', description: 'IMAP TLS flag' },
    { field: 'imapAllowInvalidCert', key: `${MAIL_CONFIG_PREFIX}imap_allow_invalid_cert`, type: 'boolean', description: 'Allow invalid IMAP TLS certificates' },
    { field: 'imapUser', key: `${MAIL_CONFIG_PREFIX}imap_user`, type: 'string', description: 'IMAP username' },
    { field: 'imapPass', key: `${MAIL_CONFIG_PREFIX}imap_pass`, type: 'string', secret: true, description: 'IMAP password' },
...
const DEFAULT_MAIL_ADMIN_CONFIG = {
    enabled: false,
    smtpHost: '',
    smtpPort: 25,
    smtpSecure: false,
...
const SECRET_FIELDS = new Set(MAIL_FIELD_SPECS.filter((item) => item.secret).map((item) => item.field));
function toKeyBuffer(raw) {
    return crypto.createHash('sha256').update(raw).digest();
}
function getCurrentEncryptionKeyRaw() {
    const raw = process.env.MAIL_CONFIG_ENCRYPTION_KEY ||
...
function getLegacyEncryptionKeyRaw() {
...
function assertCurrentEncryptionKeyReady() {
    const keyRaw = getCurrentEncryptionKeyRaw();
    if (!keyRaw) {
        throw new Error('mail config encryption key unavailable: set MAIL_CONFIG_ENCRYPTION_KEY or MODERATION_ENCRYPTION_KEY');
    }
    if (keyRaw.length < MIN_ENCRYPTION_KEY_LENGTH) {
        throw new Error(`mail config encryption key too short: minimum ${MIN_ENCRYPTION_KEY_LENGTH} characters required`);
    }
}
...
function buildEnvFallbackAdminConfig() {
    ...
    return {
        ...
        smtpUser,
        smtpPass,
        fromName: normalizeString(process.env.MAIL_FROM_NAME) || normalizeString(process.env.BRAND_NAME),
        emailFrom,
        replyTo: normalizeString(process.env.MAIL_REPLY_TO) || normalizeString(process.env.CONTACT_EMAIL),
        imapHost: normalizeString(process.env.IMAP_HOST) || smtpHost,
        imapPort: parseNumber(normalizeString(process.env.IMAP_PORT), 993),
        imapSecure: normalizeString(process.env.IMAP_SECURE) !== 'false',
        imapAllowInvalidCert: process.env.IMAP_TLS_REJECT_UNAUTHORIZED === 'false',
        imapUser: normalizeString(process.env.IMAP_USER) || smtpUser,
        imapPass: normalizeString(process.env.IMAP_PASS) || smtpPass,
...
async function getStoredMailConfig() {
    ...
    const config = { ...DEFAULT_MAIL_ADMIN_CONFIG };
    for (const spec of MAIL_FIELD_SPECS) {
        ...
        if (spec.type === 'boolean') {
            config[spec.field] = parseBoolean(raw);
        }
        else if (spec.type === 'number') {
            config[spec.field] = parseNumber(raw, DEFAULT_MAIL_ADMIN_CONFIG[spec.field]);
        }
        else {
            config[spec.field] = normalizeString(raw);
        }
    }
    return {
        config,
...
export async function getMailConfigForAdmin() {
    ...
    const effective = {
        ...
        transport: runtime.transport.kind === 'smtp'
            ? {
                ...runtime.transport,
...
export async function saveMailConfig(input) {
    ...
    for (const spec of MAIL_FIELD_SPECS) {
        ...
        const shouldDelete = spec.type === 'string'
        ...
        const serialized = spec.type === 'boolean'
            ? String(Boolean(value))
            : spec.type === 'number'
                ? String(parseNumber(String(value), DEFAULT_MAIL_ADMIN_CONFIG[spec.field]))
                : normalizeString(value);
...
export async function getEffectiveMailRuntime() {
    ...
    const adminConfig = hasStoredConfig
        ...
        : envFallback;
    if (!adminConfig.imapHost)
        adminConfig.imapHost = adminConfig.smtpHost;
    if (!adminConfig.imapUser)
        adminConfig.imapUser = adminConfig.smtpUser;
    if (!adminConfig.imapPass)
        adminConfig.imapPass = adminConfig.smtpPass;
    ...
    const emailBaseUrl = normalizeString(adminConfig.emailBaseUrl) ||
        envFallback.emailBaseUrl ||
        normalizeString(process.env.FRONTEND_URL) ||
        '';
    let transport = { kind: 'none', source: hasStoredConfig && !systemEnabled ? 'disabled' : 'none' };
    if (!hasStoredConfig || systemEnabled) {
        ...
        if (smtpHost && emailFrom) {
            transport = {
                ...
                from: emailFrom,
                tlsRejectUnauthorized: process.env.SMTP_TLS_REJECT_UNAUTHORIZED === 'false' || isLoopbackHost(smtpHost) || Boolean(adminConfig.smtpAllowInvalidCert)
                    ? false
                    : undefined,
            };
        }
        else if (!hasStoredConfig && normalizeString(process.env.BREVO_API_KEY)) {
            const from = normalizeString(process.env.BREVO_SMTP_FROM) ||
                normalizeString(process.env.EMAIL_FROM) ||
                normalizeString(process.env.BREVO_SMTP_LOGIN);
            if (from) {
                transport = {
...
//# sourceMappingURL=mailConfigService.js.map