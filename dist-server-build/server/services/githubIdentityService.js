...
function normalizeProfile(profile) {
    ...
    if (!/^\d+$/.test(githubId))
        throw new GitHubIdentityConflictError('Invalid GitHub provider identity');
    if (!email || !email.includes('@'))
        throw new GitHubIdentityConflictError('GitHub did not provide a verified email');
    return { ...profile, githubId, email };
}
function isUniqueConstraintError(error) {
    return Boolean(error && typeof error === 'object' && 'code' in error && error.code === 'P2002');
}
export async function upsertGitHubIdentity(prisma, rawProfile, resolveUsername) {
...
//# sourceMappingURL=githubIdentityService.js.map