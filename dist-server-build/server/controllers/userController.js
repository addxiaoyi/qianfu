...
import { toSafeUser, sendSuccess, sendPaginated } from '../utils/response.js';
...
import { profileUpdateSchema, bioVersionQuerySchema } from '../utils/validation.js';
import { parseJsonArray, parseJsonObject, stringifyJsonField } from '../utils/jsonField.js';
/**
 ...
 */
export const listBioVersions = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            throw new AppError('Authentication required', 401, ErrorCode.UNAUTHORIZED);
