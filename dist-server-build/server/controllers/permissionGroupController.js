...
import { PERMISSION_GROUPS, PermissionGroupManager } from '../config/permissionGroups.js';
import { invalidateUserCache } from '../middleware/auth.js';
import { AppError, ErrorCode } from '../utils/errors.js';
import { logDataChange } from '../services/auditService.js';
import { sendBatchResponse, sendDetailResponse, sendListResponse, sendUpdatedResponse, } from '../utils/response.js';
import { assignPermissionGroupSchema, batchAssignPermissionGroupSchema, idParamSchema, permissionHistoryQuerySchema } from '../utils/validation.js';
import { parseJsonArray } from '../utils/jsonField.js';
export const getAllPermissionGroups = async (req, res, next) => {
    try {
        if (!req.user) {
...
export const assignPermissionGroup = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
        }
        ...
        const validation = PermissionGroupManager.validateGroupAssignment(targetUser.role, group, assignerPermissions, req.user.role, req.isAdmin);
        if (!validation.valid) {
...
export const batchAssignPermissionGroups = async (req, res, next) => {
    try {
        if (!req.user) {
        ...
        return sendBatchResponse(res, results.map((r) => {
...
export const getPermissionHistory = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Unauthorized', 401, ErrorCode.UNAUTHORIZED);
        }
...
export const getPermissionStats = async (req, res, next) => {
    try {
        ...
        const groupStats = await prisma.user.groupBy({
            ...
            _count: { id: true },
            where: {
                role: { in: Object.keys(PERMISSION_GROUPS) }
            }
        });
...
//# sourceMappingURL=permissionGroupController.js.map