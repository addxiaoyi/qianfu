...
import { PLAN_PRICES_FEN, buildProjectScopedPaymentId, completeExternalPayment, createExternalPaymentByProvider, getPaymentProjectConfig, normalizePlanId, parseProjectKeyFromPaymentId, } from './paymentController';
const PAYMENT_PROJECT_CONFIG_PREFIX = 'payment_project:';
const SUPPORTED_UPSTREAM_PROVIDER_ORDER = ['creem', 'paypal', 'qiupay', 'xpay', 'tpay', 'hupijiao', 'paypro'];
...
const isVmqBaseUrl = (value) => {
    if (!hasText(value))
        return false;
    try {
        return /(^|\.)v\.0st\.top$/i.test(new URL(String(value)).hostname);
    }
    catch {
        return false;
    }
...
const validateProviderConfig = (config, provider, label) => {
    ...
    if (provider === 'paypal') {
        if (!hasText(config.paypalClientId) || !hasText(config.paypalClientSecret)) {
            throw new AppError(`${prefix}PayPal requires paypalClientId and paypalClientSecret`, 400, ErrorCode.VALIDATION_ERROR);
        }
        return;
    }
    if (provider === 'qiupay') {
        const isVmq = isVmqBaseUrl(config.qiupayBaseUrl);
        if (!hasText(config.qiupayBaseUrl) || !hasText(config.qiupayKey) || (!isVmq && !hasText(config.qiupayPid))) {
            throw new AppError(`${prefix}${isVmq ? 'V免签 requires qiupayBaseUrl and qiupayKey' : 'QiuPay requires qiupayBaseUrl, qiupayPid and qiupayKey'}`, 400, ErrorCode.VALIDATION_ERROR);
        }
    }
};
...
const buildProviderReadiness = (config) => {
    ...
    return {
        ...
        paypal: hasText(config.paypalClientId) && hasText(config.paypalClientSecret),
        qiupay: hasText(config.qiupayBaseUrl) && hasText(config.qiupayKey) && (isVmqBaseUrl(config.qiupayBaseUrl) || hasText(config.qiupayPid)),
    };
};
...
export const buildGlobalStatus = (env = process.env) => ({
    ...
    paypro: {
        configured: hasText(env.PAYPRO_API_URL) && hasText(env.PAYPRO_OPENAPI_SECRET),
        notifyUrl: env.PAYPRO_NOTIFY_URL || null,
    },
    xpay: {
        tenantGatewayConfigured: hasText(env.XPAY_GATEWAY_BASE_URL) && hasText(env.XPAY_TENANT_KEY) && hasText(env.XPAY_TOKEN),
        legacyConfigured: hasText(env.XPAY_API_URL) && hasText(env.XPAY_TOKEN) && hasText(env.XPAY_NOTIFY_URL),
        officialAlipayEnabled: String(env.XPAY_PROVIDER_ALIPAY_ENABLED || 'false').toLowerCase() === 'true',
        officialWechatEnabled: String(env.XPAY_PROVIDER_WECHAT_ENABLED || 'false').toLowerCase() === 'true',
        officialAlipayVerifyEnabled: String(env.XPAY_PROVIDER_ALIPAY_VERIFY_ENABLED || 'false').toLowerCase() === 'true',
        officialWechatVerifyEnabled: String(env.XPAY_PROVIDER_WECHAT_VERIFY_ENABLED || 'false').toLowerCase() === 'true',
    },
    tpay: {
        configured: hasText(env.TPAY_GATEWAY_URL) && hasText(env.TPAY_APP_ID) && hasText(env.TPAY_APP_SECRET),
        queryConfigured: hasText(env.TPAY_QUERY_URL),
    },
    hupijiao: {
        configured: hasText(env.HUPIJIAO_GATEWAY_URL) && hasText(env.HUPIJIAO_APP_ID) && hasText(env.HUPIJIAO_APP_SECRET),
        backupGatewayConfigured: hasText(env.HUPIJIAO_BACKUP_GATEWAY_URL),
        notifyConfigured: hasText(env.HUPIJIAO_NOTIFY_URL),
    },
    creem: {
        configured: hasText(env.CREEM_API_KEY)
            && hasText(env.CREEM_WEBHOOK_SECRET)
            && (hasText(env.CREEM_PRODUCT_ID) || hasText(env.CREEM_PRODUCT_MAP_JSON)),
        apiBaseUrl: env.CREEM_API_BASE_URL || null,
        returnUrl: env.CREEM_RETURN_URL || null,
    },
    qiupay: {
        configured: hasText(env.QIUPAY_BASE_URL)
            && hasText(env.QIUPAY_KEY)
            && (isVmqBaseUrl(env.QIUPAY_BASE_URL) || hasText(env.QIUPAY_PID)),
        notifyUrl: env.QIUPAY_NOTIFY_URL || null,
        returnUrl: env.QIUPAY_RETURN_URL || null,
    },
    paypal: {
        configured: hasText(env.PAYPAL_CLIENT_ID) && hasText(env.PAYPAL_CLIENT_SECRET),
        returnUrl: env.PAYPAL_RETURN_URL || null,
...
export const buildPaymentProjectConfig = (projectKey, body) => ({
    key: projectKey,
    ...
    backupUpstreamProvider: textField(body, 'backupUpstreamProvider')?.toLowerCase(),
    downstreamNotifyUrl: textField(body, 'downstreamNotifyUrl'),
    downstreamNotifySecret: textField(body, 'downstreamNotifySecret'),
    bridgeNotifySecret: textField(body, 'bridgeNotifySecret'),
    personalQrListenerSecret: textField(body, 'personalQrListenerSecret'),
    payProApiUrl: textField(body, 'payProApiUrl'),
    payProOpenApiSecret: textField(body, 'payProOpenApiSecret'),
    payProNotifyUrl: textField(body, 'payProNotifyUrl'),
    xpayApiUrl: textField(body, 'xpayApiUrl'),
    xpayToken: textField(body, 'xpayToken'),
    xpayNotifyUrl: textField(body, 'xpayNotifyUrl'),
    xpayGatewayBaseUrl: textField(body, 'xpayGatewayBaseUrl'),
    xpayGatewayNotifySecret: textField(body, 'xpayGatewayNotifySecret'),
    xpayTenantKey: textField(body, 'xpayTenantKey'),
    xpayTenantCallbackSecret: textField(body, 'xpayTenantCallbackSecret'),
    creemApiBaseUrl: textField(body, 'creemApiBaseUrl'),
    creemApiKey: textField(body, 'creemApiKey'),
    creemWebhookSecret: textField(body, 'creemWebhookSecret'),
    creemProductId: textField(body, 'creemProductId'),
    creemReturnUrl: textField(body, 'creemReturnUrl'),
    creemMode: textField(body, 'creemMode'),
    creemProductMap: textField(body, 'creemProductMap'),
    paypalClientId: textField(body, 'paypalClientId'),
    paypalClientSecret: textField(body, 'paypalClientSecret'),
    paypalMode: textField(body, 'paypalMode'),
    paypalApiBaseUrl: textField(body, 'paypalApiBaseUrl'),
    paypalReturnUrl: textField(body, 'paypalReturnUrl'),
    paypalCancelUrl: textField(body, 'paypalCancelUrl'),
    paypalExchangeRateCnyPerUsd: Number(body.paypalExchangeRateCnyPerUsd) || undefined,
    qiupayBaseUrl: textField(body, 'qiupayBaseUrl'),
    qiupayPid: textField(body, 'qiupayPid'),
    qiupayKey: textField(body, 'qiupayKey'),
    qiupayNotifyUrl: textField(body, 'qiupayNotifyUrl'),
    qiupayReturnUrl: textField(body, 'qiupayReturnUrl'),
    tpayGatewayUrl: textField(body, 'tpayGatewayUrl'),
    tpayAppId: textField(body, 'tpayAppId'),
    tpayAppSecret: textField(body, 'tpayAppSecret'),
    tpayQueryUrl: textField(body, 'tpayQueryUrl'),
    hupijiaoGatewayUrl: textField(body, 'hupijiaoGatewayUrl'),
    hupijiaoBackupGatewayUrl: textField(body, 'hupijiaoBackupGatewayUrl'),
    hupijiaoAppId: textField(body, 'hupijiaoAppId'),
    hupijiaoAppSecret: textField(body, 'hupijiaoAppSecret'),
    hupijiaoNotifyUrl: textField(body, 'hupijiaoNotifyUrl'),
    hupijiaoReturnUrl: textField(body, 'hupijiaoReturnUrl'),
    hupijiaoPlugins: textField(body, 'hupijiaoPlugins'),
    hupijiaoVersion: textField(body, 'hupijiaoVersion'),
});
export const buildQiuPayDiagnosticTests = (config) => {
    const tests = [
        {
            name: 'qiupayBaseUrl',
            ok: hasText(config.qiupayBaseUrl),
            detail: String(config.qiupayBaseUrl || 'missing'),
        },
        {
            name: 'qiupayKey',
            ok: hasText(config.qiupayKey),
            detail: hasText(config.qiupayKey) ? 'configured' : 'missing',
        },
    ];
    if (!isVmqBaseUrl(config.qiupayBaseUrl)) {
        tests.push({
            name: 'qiupayPid',
...
export const upsertPaymentProject = async (req, res, next) => {
    try {
        ...
        const config = buildPaymentProjectConfig(projectKey, body);
        if (!SUPPORTED_UPSTREAM_PROVIDERS.has(config.upstreamProvider)) {
            throw new AppError('upstreamProvider must be paypro, xpay, tpay, hupijiao, creem, qiupay or paypal', 400, ErrorCode.VALIDATION_ERROR);
        }
        if (config.backupUpstreamProvider && !SUPPORTED_UPSTREAM_PROVIDERS.has(config.backupUpstreamProvider)) {
            throw new AppError('backupUpstreamProvider must be paypro, xpay, tpay, hupijiao, creem, qiupay or paypal', 400, ErrorCode.VALIDATION_ERROR);
        }
...
export const getPaymentProjectDiagnostics = async (req, res, next) => {
    try {
        ...
        if (config.upstreamProvider === 'qiupay' || config.backupUpstreamProvider === 'qiupay') {
            tests.push(...buildQiuPayDiagnosticTests(config));
        }
...
export const uploadPaymentProjectXpayTenantQr = async (req, res, next) => {
    try {
        ...
        form.append('file', new Blob([new Uint8Array(fileBuffer)], { type: file.mimetype || 'application/octet-stream' }), file.originalname || `${payType}.png`);
        ...
        if (req.file?.path)
            await fs.rm(path.resolve(req.file.path), { force: true }).catch(() => undefined);
    }
};
export const createPaymentProjectTestOrder = async (req, res, next) => {
    try {
        ...
        try {
            ...
            throw new AppError('amount must have at most 2 decimal places', 400, ErrorCode.VALIDATION_ERROR);
        }
...
export const simulatePaymentProjectOrderSuccess = async (req, res, next) => {
    try {
        ...
        const projectConfig = await getPaymentProjectConfig(projectKey);
        const result = await completeExternalPayment(orderId, {
