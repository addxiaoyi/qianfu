...
export function isAdministrativeRole(role) {
    return typeof role === 'string' && ADMINISTRATIVE_ROLES.has(role);
}
function getRolePermissions(role) {
    if (typeof role !== 'string' || !Object.prototype.hasOwnProperty.call(PERMISSION_GROUPS, role)) {
        return null;
    }
    return PERMISSION_GROUPS[role].permissions;
}
function isPermissionAllowedForRole(permission, role) {
    const allowedRoles = PRIVILEGED_PERMISSION_ROLES[permission];
    return !allowedRoles || (typeof role === 'string' && allowedRoles.has(role));
}
