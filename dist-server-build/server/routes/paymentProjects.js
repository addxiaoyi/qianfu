...
import { listPaymentProjects, getPaymentProjectDiagnostics, getPaymentProjectXpayTenant, syncPaymentProjectXpayTenant, uploadPaymentProjectXpayTenantQr, upsertPaymentProject, deletePaymentProject, createPaymentProjectTestOrder, getPaymentProjectOrder, } from '../controllers/paymentProjectController';
const router = Router();
const upload = multer({
    storage: multer.diskStorage({
        destination: (_req, _file, cb) => cb(null, os.tmpdir()),
        filename: (_req, file, cb) => cb(null, `qianfu-xpay-qr-${Date.now()}-${file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_')}`),
    }),
    limits: {
