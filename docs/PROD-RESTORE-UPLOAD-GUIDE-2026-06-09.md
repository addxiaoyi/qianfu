...
当前公网已恢复。2026-06-09 15:08 本机执行 `npm run prod:verify:public:win` 通过，`failed_count=0`，报告为 `output/prod-public-verify/verify-public-20260609-150833.json`。

- `https://mc-u.top/api/health` -> `200`
- `https://mc-u.top/api/ready` -> `200`
- `https://mc-u.top/` 引用当前入口 `/assets/index-CHZmvcH-.js`
- 本地当前入口是 `/assets/index-CHZmvcH-.js`
- `https://mc-u.top/qianfu-dist-manifest.json` 返回 JSON manifest，且匹配本地 dist
- `https://pay.star-web.top/` 返回 `qianfu-pay-gateway`
- `pay.star-web.top` TLS 证书 CN/SAN 均为 `pay.star-web.top`
...
结论更新：15:08 已通过可用 SSH 密码完成修复。以下通道说明保留为后续应急入口：
...
如果需要非交互密码登录，可使用本地 Python 执行器。先把密码放到环境变量中，推荐使用 Base64 环境变量，避免命令行直接显示明文：

```powershell
$env:QF_SSH_USER = "root"
$env:QF_SSH_PASSWORD_B64 = "<base64-password>"
npm run prod:restore:ssh:password -- --check-only
npm run prod:restore:ssh:password -- --dry-run
npm run prod:restore:ssh:password
Remove-Item Env:\QF_SSH_PASSWORD_B64
```

作用域参数同样可用：

```powershell
npm run prod:restore:ssh:password -- --web-only
npm run prod:restore:ssh:password -- --pay-only
```

2026-06-09 15:08 复核：后续收到的 SSH 密码已对 `root@103.236.92.10` 验证成功，并已用密码版 Python 执行器完成生产修复。FTP 端口虽开放，但 Pure-FTPd 曾返回 `421 Unable to read the indexed puredb file`，不建议作为恢复通道优先选择。
...
tar -xzf /path/to/qianfu-prod-restore-20260609-ssh-ready.tar.gz -C /www/wwwroot/qianfu-app
...
## 产物三：移动端 UI 全量恢复包（前端 dist + 部署脚本，无后端）

适用：生产机 SSH/终端可达、仅需替换前端 dist 与 Nginx 配置，不重启后端 API。

文件：

```text
output/prod-restore-bundles/qianfu-prod-restore-20260611-mobile-ui.tar.gz
```

校验：

```text
24e64dc25310035fb76d58d5717c1a577017df93b77b08ecadf4a31579d06351
```

内容包含：

- `qianfu-liandeng/dist`（最新手机端 UI 修复后构建）
- `deploy/nginx/*.conf.example`（Nginx 模板）
- `docs/`（当前操作文档快照）
- `package.json` / `package-lock.json`

不含后端 (`dist-server/`、`prisma/`、`server/`)，不会影响 PM2 `qianfu-api` 进程。

生产机解包：

```bash
cd /www/wwwroot/qianfu-app
tar -xzf /path/to/qianfu-prod-restore-20260611-mobile-ui.tar.gz -C /www/wwwroot/qianfu-app
```

或通过 SSH 本机脚本上传（需 `QF_SSH_PASSWORD_B64` 环境变量）：

```powershell
# 先确认当前 ssh 可达
# 以下仅为参考，实际执行需先恢复生产入口 80/443/22
npm run prod:restore:ssh:password -- --web-only
```

注意：2026-06-11 本包生成时生产机 `103.236.92.10` 的 `22/80/443` 均不可达，SSH/SCP 上传当前无法执行。需先恢复生产机网络入口后，再将此包上传解包。
