...
import { getMinotarAvatarUrl } from '@/utils/minecraftAvatar';
...
const ThreeDHeadShowcase: React.FC<ThreeDHeadShowcaseProps> = ({ username }) => {
  ...
  const remoteAvatarEnabled = import.meta.env.VITE_ENABLE_MINOTAR_AVATAR === 'true';
  const imageSrc = getMinotarAvatarUrl(player, remoteAvatarEnabled, imageSize);
    ...
    <div className="relative overflow-hidden rounded-[1.5rem] border border-zinc-100 bg-white p-3 shadow-[0_12px_40px_rgba(0,0,0,0.04)]">
      ...
      <div className="relative space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-[9px] font-black uppercase tracking-[0.22em] text-zinc-400">角色皮肤</div>
          <div className="rounded-full bg-black px-2 py-1 text-[9px] font-black uppercase tracking-[0.18em] text-white">{remoteOk ? '实时' : '本地'}</div>
        </div>
        ...
        <p className="text-center text-[9px] font-black uppercase tracking-[0.18em] text-zinc-400">玩家：{player}</p>
      </div>
    </div>
  );
};
